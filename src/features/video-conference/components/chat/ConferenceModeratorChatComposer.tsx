...
export function ConferenceModeratorChatComposer({
  message,
  canSend,
  busy,
  errorMessage,
      ...
      <form onSubmit={submit} className="flex items-end gap-2 p-3">
        <div className="min-w-0 flex-1">
          <textarea
            ...
            placeholder={canSend ? 'پیام در کانال مدیران…' : 'دسترسی ندارید'}
            className="max-h-28 min-h-11 w-full resize-none rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm outline-none focus:border-amber-500 disabled:cursor-not-allowed disabled:opacity-60"
          />
        ...
        <button
          ...
          className="min-h-11 rounded-xl bg-amber-600 px-4 text-xs font-bold text-slate-950 disabled:cursor-not-allowed disabled:opacity-50"
        >
          {editing ? 'ذخیره' : 'ارسال'}
        </button>
      </form>
