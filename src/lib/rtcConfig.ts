...
export function buildRTCConfigFromDB(cfg: Record<string, string>): RTCConfiguration {
  ...
  if (turnServerUrl && username && credential) {
    // Strip any scheme prefix so we control the exact URL forms we emit
    const bare = turnServerUrl
      .replace(/^turns?:\/?\/?/i, '')
      .replace(/^turns?:/i, '')
      .split('?')[0];
    iceServers.push({
  ...
  if (!_configPromise) {
    _configPromise = supabase
      ...
      .then(({ data }) => {
        ...
        if ((built.iceServers as RTCIceServer[]).length === 0) return buildEnvFallbackConfig();
        return built;
      })
