...
const ORG_USERS_CACHE_TTL_MS = 60_000;

type CacheEntry = { expiresAt: number; users: OrgUserProfile[] };
const orgUsersCache = new Map<string, CacheEntry>();
const orgUsersInflight = new Map<string, Promise<OrgUserProfile[]>>();
...
async function loadSelectableOrgUsers(cacheKey: string): Promise<OrgUserProfile[]> {
  ...
  const request = (async () => {
    const { data, error } = await supabase.rpc('get_selectable_users_v2');
  ...
  orgUsersInflight.set(cacheKey, request);
  try {
...
function buildGroups(users: OrgUserProfile[]): OrgUnitGroup[] {
...
export function useOrgUsers(currentUserId: string | null): UseOrgUsersResult {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError(false);
      try {
        const users = await loadSelectableOrgUsers(currentUserId);
        if (cancelled) return;
        setAllUsers(users);
        setGroups(buildGroups(users));
      } catch (err) {
