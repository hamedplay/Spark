...
export function normalizeMeetingDate(input: string | null | undefined): string | null {
  ...
  if (jMatch) {
    ...
    const jd = Number(d);
    if (isJalaliYear(jy)) {
      // moment-jalaali strict parsing requires the input to match jMM/jDD width.
      // Canonicalize first so both 1405/5/4 and 1405/05/04 remain valid.
      const mm = moment(`${jy}/${pad2(jm)}/${pad2(jd)}`, 'jYYYY/jMM/jDD', true);
