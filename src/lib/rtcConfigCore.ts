...
export function buildRTCConfigFromDB(cfg: Record<string, string>): RTCConfiguration {
  ...
  if (!hasStun && IS_DEV) {
    ...
    log.info('[RTCConfig] Development only: public Google STUN fallback added');
  } else if (!hasStun && !hasTurnCreds) {
    log.error('[RTCConfig] Production has neither configured STUN nor TURN; connectivity may fail');
  }
...
export function buildEnvFallbackConfig(): RTCConfiguration {
  ...
  if (host && username && credential) {
    ...
    iceServers.push(FALLBACK_STUN);
    log.warn('[RTCConfig] env fallback: development Google STUN fallback enabled');
  } else {
    log.error('[RTCConfig] env fallback: production TURN/STUN configuration is missing');
  }
