 ...
 * GHSA-5pgg-2g8v-p4x9).
 *
 * ExcelJS is deliberately imported only inside read/writeFile. The rest of
 * this module is tiny synchronous data shaping, so opening Backup, Reports or
 * User Management no longer downloads the Excel engine until the user actually
...
export async function writeFile(wb: Workbook, filename: string): Promise<void> {
  const ejWb = await createExcelWorkbook();
  ...
  for (const { name, sheet } of wb._ordered) {
    ...
    if (sheet._aoa) {
      ...
      for (const row of sheet._rows) {
        ws.addRow(sheet._headers.map((h) => row[h] ?? '') as never[]);
      }
    }
  }
