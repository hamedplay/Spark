...
export function UserManagementPanel({ currentUserId }: Props) {
  ...
  const load = useCallback(async () => {
    const { data } = await supabase.from('profiles').select(
      'user_id, full_name, email, username, phone, organization, position, department, employee_id, hire_date, birth_date, gender, city, location, bio, national_id, avatar_url, is_admin, is_security_admin, security_role_version, is_active, is_hidden, account_status, created_at'
    ).neq('account_status', 'RETIRED').order('created_at', { ascending: false });
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-700/50 text-right">
                ...
                <th className="px-4 py-3 text-xs font-semibold text-gray-500 dark:text-gray-400 text-center">سطح دسترسی</th>
              ...
              {filtered.map(p => (
                <tr key={p.user_id} onClick={() => openPanel('preview', p)}
                  ...
                  <td className="px-4 py-3">
                    ...
                    {p.username && <div className="text-xs text-teal-600 dark:text-teal-400 font-medium mt-0.5">@{p.username}</div>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    {p.is_security_admin ? (
                      <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">Security Admin</span>
                    ) : p.is_admin ? (
                      <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">Admin</span>
                    ) : (
                      <span className="inline-flex px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">کاربر عادی</span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-center">
                  ...
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-1" onClick={e => e.stopPropagation()}>
                      {p.user_id !== currentUserId && (
                        <AdminUserDeleteAction userId={p.user_id} fullName={p.full_name} email={p.email} onDeleted={load} />
                      )}
                ...
                <tr>
                  <td colSpan={7} className="py-14">
                    <div className="flex flex-col items-center gap-3 text-gray-400">
                      ...
                      <p className="text-sm">{search ? `کاربری با عنوان «${search}» یافت نشد` : 'هنوز کاربری ثبت نشده است'}</p>
                      <button
