...
const inp = 'w-full px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-hidden focus:ring-2 focus:ring-teal-500 focus:border-transparent transition text-sm';
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5 space-y-4">
        ...
        <div className="flex items-center gap-3 pt-1">
          <button type="button" onClick={() => set('is_active', !form.is_active)}
            className={`w-11 h-6 rounded-full relative transition-colors shrink-0 ${form.is_active ? 'bg-teal-500' : 'bg-gray-200 dark:bg-gray-600'}`}>
      ...
      {testResult && (
        <div className={`flex items-center gap-2 px-4 py-3 rounded-2xl border text-sm ${testResult.ok ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 text-green-700 dark:text-green-300' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800 text-red-700 dark:text-red-300'}`}>
          {testResult.ok ? <Wifi className="w-4 h-4 shrink-0" /> : <WifiOff className="w-4 h-4 shrink-0" />}
          {testResult.msg}
        </div>
      )}
...
function SendTab() {
  ...
  const send = async () => {
    ...
    setResult(null);
    try {
      const d = await callRahyab('send', { mobiles: [mobile.trim()], message: message.trim() });
      if (d.ok) {
