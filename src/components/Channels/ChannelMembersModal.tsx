  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[60]" onClick={onClose}>
      <div className="absolute inset-y-0 right-0 w-full max-w-sm bg-white dark:bg-gray-900 shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        {showPicker && (
          <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex-shrink-0 bg-teal-50 dark:bg-teal-900/10">
            <input
              autoFocus
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="جستجوی کاربر..."
              className="w-full px-3 py-2 text-sm border border-gray-200 dark:border-gray-600 rounded-xl dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-teal-500/40"
              dir="rtl"
            />
              ...
              {search ? (
                filteredNonMembers.slice(0, 20).map(p => (
                  <button key={p.user_id} onClick={() => handleAdd(p.user_id)} disabled={loading === p.user_id}
                    ...
                    {loading === p.user_id && <div className="w-4 h-4 border-2 border-teal-500 border-t-transparent rounded-full animate-spin flex-shrink-0" />}
                  </button>
                ))
              ) : (
          ...
          {members.filter(m => m.profile !== null).map(m => {
            ...
            return (
              <div key={m.user_id} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800">
                {p?.avatar_url
                  ? <img src={p.avatar_url} className="w-8 h-8 rounded-full object-cover flex-shrink-0" alt="" />
                  : <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-gray-500" /></div>}
