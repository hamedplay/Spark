...
export function DecisionActionModal({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // ── Manager report: dedicated tracking RPC ─────────────────────────────
      if (action === 'report' && isManager) {
        const { data, error: rpcErr } = await supabase.rpc('record_manager_minutes_decision_report', {
      ...
      if (isManager && (action === 'update' || action === 'progress' || action === 'complete')) {
        setError('این عملیات از صفحه پیگیری مدیریتی قابل انجام نیست.');
        return;
      }

      // ── Owner operations: update_my_minutes_decision ──────────────────────
      if (!isManager && (action === 'update' || action === 'progress' || action === 'report' || action === 'complete')) {
