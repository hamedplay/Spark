...
export function DateTimeSection(props: {
  ...
  setShowManualDateTime: (v: boolean) => void;
  manualDateStr: string;
  setManualDateStr: (v: string) => void;
  setScheduleDate: (v: { jy: number; jm: number; jd: number } | null) => void;
  ...
  const {
    scheduleDate, showManualDateTime, setShowManualDateTime,
    manualDateStr, setManualDateStr, setScheduleDate,
    manualStartTime, setManualStartTime, setStartTime,
    manualEndTime, setManualEndTime, setEndTime,
    startTime, endTime,
  } = props;
  ...
  return (
    <div className="space-y-2">
      ...
      <button type="button" onClick={() => { setShowManualDateTime(!showManualDateTime); if (!manualDateStr && scheduleDate) setManualDateStr(`${scheduleDate.jy}/${String(scheduleDate.jm).padStart(2,'0')}/${String(scheduleDate.jd).padStart(2,'0')}`); if (!manualStartTime) setManualStartTime(startTime); if (!manualEndTime) setManualEndTime(endTime); }}
