...
export function ChatPanel({
  ...
  useEffect(() => {
    let cancelled = false;
    void supabase.from('conference_messages')
      .select('*')
      .eq('room_id', roomId)
      .eq('is_deleted', false)
      ...
      .then(({ data }) => {
        ...
        for (const message of data as ConferenceMessage[]) onOwnMessage(message);
      });
    return () => { cancelled = true; };
  }, [roomId, onOwnMessage, supabase]);

  // ── Typing-indicator channel ─────────────────────────────────────────────────
  useEffect(() => {
    const ch = supabase.channel(`conference:${roomId}:typing`, {
  ...
  // Resolve durable storage paths to short-lived URLs only for display.
  useEffect(() => {
    let cancelled = false;
    const unresolved = messages.filter(m => m.image_path && !m.image_url && !resolvedImages[m.id]);
    if (!unresolved.length) return;
    void Promise.all(unresolved.map(async (message) => {
      ...
      if (Object.keys(next).length) setResolvedImages(prev => ({ ...prev, ...next }));
    });
    return () => { cancelled = true; };
  }, [messages, resolvedImages, supabase]);

  // ── Send text message — DB first, then broadcast ─────────────────────────────
  const sendMessage = useCallback(async (imagePath?: string) => {
    ...
    <>
      ...
      >
        ...
        {messages.map(m => {
          ...
          const imageSrc = m.image_url || resolvedImages[m.id] || null;
          return (
