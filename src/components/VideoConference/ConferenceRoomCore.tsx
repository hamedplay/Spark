...
export function ConferenceRoomView({ room, currentUserId, currentUserName, myPeerId, localStream, onLeave, onInvite, loadRTCConfig }: Props) {
  ...
  const [presenterUserId, setPresenterUserId] = useState<string | null>(room.presenter_user_id ?? null);
  const [dbPinnedUserId, setDbPinnedUserId] = useState<string | null>(room.pinned_user_id ?? null);
  ...
  const [speakingLimitEnabled, setSpeakingLimitEnabled] = useState(room.speaking_limit_enabled ?? false);
  ...
  const toggleChatEnabled = useCallback(async () => {
    ...
    const { data, error } = await supabase.rpc('set_conference_chat_enabled', { p_room_id: room.id, p_enabled: next });
    if (error || !data?.ok) { toast.error('خطا در تغییر وضعیت چت'); return; }
    setChatEnabled(next);
  }, [chatEnabled, room.id, supabase]);

  const toggleSpeakingLimit = useCallback(async () => {
    const next = !speakingLimitEnabled;
    const { data, error } = await supabase.rpc('set_conference_speaking_limit_enabled', { p_room_id: room.id, p_enabled: next });
    if (error || !data?.ok) { toast.error('خطا در تغییر محدودیت زمان صحبت'); return; }
    setSpeakingLimitEnabled(next);
    toast(next ? 'محدودیت زمان صحبت فعال شد' : 'محدودیت زمان صحبت غیرفعال شد');
  }, [speakingLimitEnabled, room.id, supabase]);
  ...
  // Single authoritative heartbeat for authenticated and guest actors.
  useEffect(() => {
    const touch = () => supabase.rpc('conference_heartbeat', { p_room_id: room.id });
    void touch();
    const t = setInterval(() => { void touch(); }, 30_000);
    return () => clearInterval(t);
  }, [room.id, supabase]);
  ...
  // Unified RTC quality/adaptation from PeerDiagnostics snapshots.
  const healthySamplesRef = useRef(0);
  const lastAdaptationAtRef = useRef(0);
  useEffect(() => {
    const snapshots = Array.from(peerDiagnostics.values()).filter(d => Date.now() - d.timestamp < 15_000);
    ...
    for (const d of snapshots) {
      if (d.rttMs !== null) {
        latencies[d.peerId] = d.rttMs;
        worstRtt = Math.max(worstRtt, d.rttMs);
      }
      if (d.packetLossPct !== null) worstLoss = Math.max(worstLoss, d.packetLossPct);
      const peer = peersRef.current.get(d.peerId);
      if (peer) {
        peer.networkQuality =
          (d.packetLossPct ?? 0) < 1 && (d.rttMs ?? 0) < 120 ? 'excellent' :
          (d.packetLossPct ?? 0) < 3 && (d.rttMs ?? 0) < 220 ? 'good' :
          (d.packetLossPct ?? 0) < 8 && (d.rttMs ?? 0) < 450 ? 'fair' : 'poor';
      }
    }
    setPeerLatencies(latencies);
    setPeers(new Map(peersRef.current));
    setMyQuality(
      worstLoss < 1 && worstRtt < 120 ? 'excellent' :
      worstLoss < 3 && worstRtt < 220 ? 'good' :
      worstLoss < 8 && worstRtt < 450 ? 'fair' : 'poor'
    );

    const now = Date.now();
    const qualities: VideoQuality[] = ['low', 'medium', 'high'];
    const current = adaptiveQualityRef.current;
    const currentIndex = qualities.indexOf(current);
    const configuredIndex = qualities.indexOf(videoQualityRef.current);
    const degraded = worstLoss >= 5 || worstRtt >= 450;
    const healthy = worstLoss < 1.5 && worstRtt > 0 && worstRtt < 180;

    if (degraded) {
      healthySamplesRef.current = 0;
      if (currentIndex > 0 && now - lastAdaptationAtRef.current >= 10_000) {
        const next = qualities[currentIndex - 1];
        lastAdaptationAtRef.current = now;
        setAdaptiveQuality(next);
        void applyVideoConstraintsRef.current(next, dataSaverModeRef.current);
        toast('کیفیت به دلیل ضعف شبکه کاهش یافت', { icon: '📉', duration: 4000 });
      }
      return;
    }

    healthySamplesRef.current = healthy ? healthySamplesRef.current + 1 : 0;
    if (
      healthySamplesRef.current >= 3 && currentIndex < configuredIndex &&
      now - lastAdaptationAtRef.current >= 20_000
    ) {
      const next = qualities[currentIndex + 1];
      healthySamplesRef.current = 0;
      lastAdaptationAtRef.current = now;
  ...
  // ── Tiles ──────────────────────────────────────────────────────────────────
  const cameraTiles = [
    { peerId: myPeerId, userId: currentUserId, displayName: currentUserName, stream: localStream, isMuted, isVideoOff, isHandRaised, isLocal: true, isHost, networkQuality: myQuality, avatarUrl: peerAvatarUrls[currentUserId], pingMs: undefined as number | undefined, isScreenSharing: false },
    ...Array.from(peers.values()).map(p => ({ peerId: p.peerId, userId: p.userId, displayName: p.displayName, stream: p.stream, isMuted: p.isMuted, isVideoOff: p.isVideoOff, isHandRaised: p.isHandRaised, isLocal: false, isHost: hostId === p.userId, networkQuality: p.networkQuality, avatarUrl: peerAvatarUrls[p.userId], pingMs: peerLatencies[p.peerId], isScreenSharing: false })),
  ];
  const screenTiles = [
    ...(isScreenSharing && screenStreamRef.current ? [{ peerId: `${myPeerId}:screen`, userId: currentUserId, displayName: `${currentUserName} · ارائه`, stream: screenStreamRef.current, isMuted: false, isVideoOff: false, isHandRaised: false, isLocal: true, isHost, networkQuality: myQuality, avatarUrl: peerAvatarUrls[currentUserId], pingMs: undefined as number | undefined, isScreenSharing: true }] : []),
    ...Array.from(peers.values()).filter(p => p.screenStream).map(p => ({ peerId: `${p.peerId}:screen`, userId: p.userId, displayName: `${p.displayName} · ارائه`, stream: p.screenStream, isMuted: false, isVideoOff: false, isHandRaised: false, isLocal: false, isHost: hostId === p.userId, networkQuality: p.networkQuality, avatarUrl: peerAvatarUrls[p.userId], pingMs: peerLatencies[p.peerId], isScreenSharing: true })),
  ];
  const allTiles = [...screenTiles, ...cameraTiles];

  useEffect(() => {
    const desiredUser = presenterUserId || dbPinnedUserId;
    if (!desiredUser) return;
    const screen = allTiles.find(t => t.userId === desiredUser && t.isScreenSharing);
    const camera = allTiles.find(t => t.userId === desiredUser && !t.isScreenSharing);
    const desiredPeer = screen?.peerId || camera?.peerId;
    if (desiredPeer) setPinnedPeerId(desiredPeer);
  }, [presenterUserId, dbPinnedUserId, isScreenSharing, peers]);
    ...
    <div className={`flex flex-col bg-gray-950 text-white select-none relative ${isFullscreen ? 'fixed inset-0 z-[9999]' : 'h-full'}`} dir="rtl">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0 relative">
        ...
        <SidePanelContainer
          ...
          onOwnMessage={msg => setMessages(prev => prev.some(m => m.id === msg.id) ? prev : [...prev, msg])}
          allTiles={allTiles}
