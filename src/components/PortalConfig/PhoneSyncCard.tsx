...
export function PhoneSyncCard() {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/sync-profile-phone-to-auth`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, apikey: import.meta.env.VITE_SUPABASE_ANON_KEY },
        body: JSON.stringify({ user_id: selectedUser.user_id, action: 'check' }),
      });
      const json = await res.json();
      if (!res.ok) { toast.error(json.error || 'خطا در بررسی'); return; }
      setCheckResult(json);
    } catch {
  ...
  const handleSync = async () => {
    if (!selectedUser) return;
    setSyncing(true); setShowConfirm(false);
    try {
      const { data: session } = await supabase.auth.getSession();
      const token = session?.session?.access_token;
      if (!token) { toast.error('نشست معتبر نیست'); return; }
      ...
      if (!res.ok) {
        const errMap: Record<string, string> = {
          NOT_ADMIN: 'فقط ادمین می‌تواند', PROFILE_NOT_FOUND: 'پروفایل یافت نشد', PROFILE_INACTIVE: 'پروفایل غیرفعال است',
          PROFILE_PHONE_INVALID: 'شماره پروفایل نامعتبر است', AUTH_USER_NOT_FOUND: 'کاربر Auth یافت نشد',
          CONFLICT_PHONE_ON_ANOTHER_USER: 'شماره روی کاربر دیگری در Auth ثبت شده', AUTH_PHONE_CONFLICT: 'کاربر Auth شماره متفاوتی دارد',
          PHONE_DUPLICATE: 'شماره تکراری است', AUTH_UPDATE_FAILED: 'خطا در به‌روزرسانی Auth',
        };
      ...
      <div className="space-y-3">
        <div className="flex gap-2">
          <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="نام، ایمیل یا شماره..."
        ...
        {searchResults.length > 0 && (
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {searchResults.map(p => (
              <button key={p.user_id} onClick={() => { setSelectedUser(p); setCheckResult(null); }}
        ...
        {selectedUser && (
          <div className="pt-3 border-t border-gray-100 dark:border-gray-700 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300">{selectedUser.full_name || 'بدون نام'}</p>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowConfirm(true)} disabled={syncing}
        ...
        {checkResult && (
          <div className="pt-3 border-t border-gray-100 dark:border-gray-700 space-y-1.5">
            ...
            {checkResult.auth_phone_masked !== undefined && (
              <p className="text-xs text-gray-500 dark:text-gray-400">شماره Auth: {checkResult.auth_phone_masked || 'ثبت نشده'}</p>
            )}
            {checkResult.already_synced && (<p className="text-xs text-green-600 dark:text-green-400 font-medium">همگام‌سازی قبلاً انجام شده</p>)}
            {checkResult.conflict && (<p className="text-xs text-red-600 dark:text-red-400 font-medium">تداخل: شماره روی کاربر دیگری در Auth</p>)}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-sm w-full space-y-4">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowConfirm(false)}
                  className="flex-1 px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 rounded-xl text-sm transition-colors">انصراف</button>
