...
import { KeyRound, Loader as Loader2, ShieldCheck, X } from 'lucide-react';
...
export function PasswordRecoveryCard() {
  ...
  const handleToggle = async (v: boolean) => {
    ...
    if (row?.success !== true || error) {
      const errMap: Record<string, string> = {
        NOT_AUTHENTICATED: 'احراز هویت نشده', NOT_ADMIN: 'فقط ادمین می‌تواند', PROVIDER_NOT_READY: 'سرویس‌دهنده فعال نیست',
        TEMPLATE_NOT_READY: 'قالب پیامک بازیابی ساخته نشده', SECRET_NOT_CONFIRMED: 'Secret بازیابی تأیید نشده',
        E2E_NOT_VERIFIED: 'تست E2E بازیابی انجام نشده', INVALID_TTL: 'TTL نامعتبر', TEST_MODE_STILL_ACTIVE: 'ابتدا حالت تست را غیرفعال کنید',
      };
  ...
  const handleTestModeToggle = async (v: boolean) => {
    setTestSaving(true);
    const { data, error } = await supabase.rpc('set_phone_password_recovery_test_mode', { p_enabled: v, p_test_phone: v ? testPhoneInput : '' });
    const row = Array.isArray(data) ? data[0] : data;
    if (row?.success !== true || error) {
      const errMap: Record<string, string> = {
        NOT_AUTHENTICATED: 'احراز هویت نشده', NOT_ADMIN: 'فقط ادمین می‌تواند', PROVIDER_NOT_READY: 'سرویس‌دهنده فعال نیست',
        TEMPLATE_NOT_READY: 'قالب پیامک بازیابی ساخته نشده', SECRET_NOT_CONFIRMED: 'Secret بازیابی تأیید نشده',
        INVALID_TTL: 'TTL نامعتبر', INVALID_PHONE: 'شماره موبایل نامعتبر', PROFILE_NOT_FOUND: 'شماره در پروفایل فعال ثبت نشده است',
        ...
        AUTH_PHONE_DUPLICATE: 'شماره در Auth تکراری است', AUTH_USER_NOT_FOUND: 'کاربر Auth برای این پروفایل یافت نشد',
        AUTH_PROFILE_MISMATCH: 'شماره Auth متعلق به Profile دیگری است', PHONE_NOT_UNIQUE: 'شماره موبایل منحصر به فرد نیست',
    ...
    setTestMode(v); setMaskedPhone(row?.masked_phone || ''); setTestSaving(false);
  ...
  const handleConfirmSecret = async () => {
    ...
    try {
      const { data: result } = await supabase.functions.invoke('check-phone-password-reset-runtime', { body: {} });
      if (result?.ok === true && result?.secret_configured === true && result?.origins_configured === true && result?.runtime_confirmed === true) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
      <div className="flex items-center justify-between gap-3">
        ...
        {isAdmin && (
          <button onClick={() => handleToggle(!enabled)} disabled={saving || testMode}
      ...
      {isAdmin && !secretConfirmed && (
        <button onClick={handleConfirmSecret} disabled={secretSaving}
      ...
      {isAdmin && (
        <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <button onClick={() => handleTestModeToggle(!testMode)} disabled={testSaving || (testMode ? false : !testReady)}
          ...
          {testMode && maskedPhone && (<p className="text-xs text-gray-500 dark:text-gray-400 mb-2">شماره تست: {maskedPhone}</p>)}
          {!testMode && (
            <input type="tel" value={testPhoneInput} onChange={e => setTestPhoneInput(e.target.value)} placeholder="مثال: 09123456789" dir="ltr"
