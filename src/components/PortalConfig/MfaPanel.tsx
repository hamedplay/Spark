      ...
      <div className="flex flex-col gap-4">
        {qrCode && (
          <div className="flex flex-col items-center gap-3">
            ...
            {secret && (
              <p className="text-xs text-gray-500 dark:text-gray-400 text-center font-mono bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-lg break-all">{secret}</p>
            )}
          </div>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setStep('idle'); setQrCode(null); setSecret(null); setCode(''); }}
            className="px-4 py-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-600 dark:text-gray-300 rounded-xl text-sm transition-colors">انصراف</button>
        </div>
      </div>
    );
  }
