import React, { useState, useEffect, useCallback } from 'react';
import { Smartphone, KeyRound, CircleCheck as CheckCircle2, Circle as XCircle, Loader as Loader2, RefreshCw } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { invokeEdgeFunctionWithTimeout } from '../../lib/invokeEdgeFunction';
import { SecurityStepUpModal, useSecurityStepUp } from '../../features/security-settings';
import toast from 'react-hot-toast';

interface AdminStatus {
  ok: boolean;
  error?: string;
  phone_login_canonical_enabled?: boolean;
  phone_login_canonical_ready?: boolean;
  phone_password_recovery_canonical_enabled?: boolean;
  phone_password_recovery_canonical_ready?: boolean;
  provider_selected?: boolean;
  provider_active?: boolean;
  login_template_ready?: boolean;
  recovery_template_ready?: boolean;
  recovery_ttl_valid?: boolean;
  recovery_ttl_seconds?: number;
  recovery_secret_configured?: boolean;
  origins_set?: boolean;
  origins_count?: number;
  sync_matched?: number;
  sync_auth_only?: number;
  sync_profile_only?: number;
  sync_mismatched?: number;
  sync_duplicates?: number;
  last_dispatch?: { status: string; created_at: string; event_type: string } | null;
}

interface RecoveryRuntimeStatus {
  ok?: boolean;
  secret_configured?: boolean;
  origins_configured?: boolean;
  runtime_confirmed?: boolean;
  error?: string;
}

const DISPATCH_STATUS_LABELS: Record<string, string> = {
  sent: 'ارسال‌شده',
  delivered: 'تحویل‌شده',
  failed: 'ناموفق',
  pending: 'در انتظار ارسال',
  skipped: 'ارسال‌نشده',
};

const EVENT_TYPE_LABELS: Record<string, string> = {
  login_otp: 'کد ورود',
  password_recovery_otp: 'کد بازیابی رمز عبور',
  registration_otp: 'کد ثبت‌نام',
};

const PHONE_AUTH_ERROR_MESSAGES: Record<string, string> = {
  SECURITY_ADMIN_REQUIRED: 'تغییر مسیرهای احراز هویت فقط برای Security Admin مجاز است.',
  STEPUP_REQUIRED: 'برای این تغییر تأیید TOTP لازم است.',
  SESSION_REQUIRED: 'نشست امنیتی در دسترس نیست. دوباره وارد شوید.',
  SESSION_INVALID: 'نشست امنیتی معتبر نیست. دوباره وارد شوید.',
  NO_EFFECTIVE_CHANGE: 'تغییری برای ذخیره وجود ندارد.',
};

function StatusBadge({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span className={`inline-flex items-center gap-1 text-xs font-medium ${ok ? 'text-green-600 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`}>
      {ok ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
      {label}
    </span>
  );
}

function PhoneAuthCard() {
  const [status, setStatus] = useState<AdminStatus | null>(null);
  const [loading, setLoading] = useState(true);
  const [toggling, setToggling] = useState(false);
  const [statusMessage, setStatusMessage] = useState('');
  const [canManageSecurity, setCanManageSecurity] = useState(false);
  const stepUp = useSecurityStepUp({
    purpose: 'auth_settings_change',
    title: 'تأیید تغییر مسیر احراز هویت',
    description: 'فعال یا غیرفعال‌کردن ورود و بازیابی OTP یک تغییر امنیتی حساس است و به تأیید TOTP نیاز دارد.',
    confirmLabel: 'تأیید و اعمال تغییر',
  });

  const load = useCallback(async () => {
    setLoading(true);
    setStatusMessage('');
    try {
      try {
        const runtime = await invokeEdgeFunctionWithTimeout<RecoveryRuntimeStatus>(
          'check-phone-password-reset-runtime',
          {},
          20_000,
        );
        if (runtime.ok !== true || runtime.runtime_confirmed !== true) {
          setStatusMessage('پیش‌نیاز امن بازیابی رمز کامل نیست. کلید امن و نشانی‌های مجاز را بررسی کنید.');
        }
      } catch {
        setStatusMessage('بررسی خودکار پیش‌نیاز بازیابی انجام نشد؛ وضعیت ذخیره‌شده نمایش داده می‌شود.');
      }

      const [statusResult, securityAdminResult] = await Promise.all([
        supabase.rpc('get_phone_auth_admin_status'),
        supabase.rpc('is_current_security_admin'),
      ]);
      if (statusResult.error) throw statusResult.error;
      const row = Array.isArray(statusResult.data) ? statusResult.data[0] : statusResult.data;
      setStatus(row as AdminStatus);
      setCanManageSecurity(!securityAdminResult.error && securityAdminResult.data === true);
    } catch {
      setStatus(null);
      setCanManageSecurity(false);
      setStatusMessage('دریافت وضعیت ورود و بازیابی انجام نشد.');
    } finally {
      setLoading(false);
    }
  }, []);

  useEffect(() => { void load(); }, [load]);

  const persistFlag = async (loginEnabled?: boolean, recoveryEnabled?: boolean) => {
    setToggling(true);
    try {
      const { data, error } = await supabase.rpc('set_phone_auth_canonical_flags', {
        p_login_enabled: loginEnabled ?? null,
        p_recovery_enabled: recoveryEnabled ?? null,
      });
      if (error) throw error;
      const result = data as { ok?: boolean; error?: string } | null;
      if (!result?.ok) throw new Error(result?.error || 'PHONE_AUTH_UPDATE_FAILED');
      await load();
      toast.success('تنظیمات امنیتی ذخیره شد');
    } finally {
      setToggling(false);
    }
  };

  const requestToggle = async (loginEnabled?: boolean, recoveryEnabled?: boolean) => {
    if (!canManageSecurity) {
      toast.error('برای تغییر این گزینه باید Security Admin باشید.');
      return;
    }
    try {
      await stepUp.requireStepUp(() => persistFlag(loginEnabled, recoveryEnabled));
    } catch (error) {
      const code = error instanceof Error ? error.message : 'PHONE_AUTH_UPDATE_FAILED';
      toast.error(PHONE_AUTH_ERROR_MESSAGES[code] || 'خطا در ذخیره تنظیمات امنیتی');
    }
  };

  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5 flex items-center justify-center">
        <Loader2 className="w-5 h-5 animate-spin text-gray-400" />
      </div>
    );
  }

  if (!status?.ok) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-5">
        <p className="text-sm text-gray-500 dark:text-gray-400">دسترسی به وضعیت ورود و بازیابی موبایلی محدود است.</p>
      </div>
    );
  }

  const loginOn = status.phone_login_canonical_enabled === true;
  const recoveryOn = status.phone_password_recovery_canonical_enabled === true;
  const loginReady = status.phone_login_canonical_ready === true;
  const recoveryReady = status.phone_password_recovery_canonical_ready === true;

  return (
    <>
      <SecurityStepUpModal
        state={stepUp.state}
        title="تأیید تغییر مسیر احراز هویت"
        description="فعال یا غیرفعال‌کردن ورود و بازیابی OTP یک تغییر امنیتی حساس است و به تأیید TOTP نیاز دارد."
        confirmLabel="تأیید و اعمال تغییر"
        onSelectFactor={stepUp.setSelectedFactorId}
        onCodeChange={stepUp.setCode}
        onClose={stepUp.close}
        onVerify={() => { void stepUp.verify(); }}
      />

      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="flex items-center gap-3 px-5 py-3.5 border-b border-gray-100 dark:border-gray-700">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400">
            <Smartphone className="w-4 h-4" />
          </div>
          <div>
            <h3 className="font-bold text-gray-800 dark:text-white">ورود و بازیابی موبایلی بدون رمز</h3>
            <p className="mt-0.5 text-xs text-gray-400">کانال‌های OTP مستقل از «ورود با موبایل و رمز عبور» در سیاست‌های امنیتی هستند.</p>
          </div>
          <button onClick={() => void load()} disabled={loading || toggling} className="mr-auto p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 transition-colors disabled:opacity-50" title="بررسی دوباره پیش‌نیازها">
            <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} />
          </button>
        </div>

        <div className="p-5 space-y-4">
          {!canManageSecurity && (
            <div className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs text-blue-700 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-300">
              وضعیت قابل مشاهده است؛ تغییر این دو مسیر فقط برای Security Admin دارای TOTP فعال مجاز است.
            </div>
          )}

          {statusMessage && (
            <div role="status" className="rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
              {statusMessage}
            </div>
          )}

          <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-600">
            <div className="flex items-center gap-2.5">
              <Smartphone className="w-4 h-4 text-teal-500" />
              <div>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-200">ورود با کد یک‌بارمصرف موبایل (OTP)</p>
                <p className="text-xs text-gray-400">ورود بدون رمز عبور؛ نمایش در صفحه ورود فقط در صورت آماده‌بودن Provider، Template و Backend</p>
              </div>
            </div>
            <button
              type="button"
              aria-label="فعال‌سازی ورود OTP موبایل"
              aria-checked={loginOn}
              role="switch"
              onClick={() => { void requestToggle(!loginOn); }}
              disabled={toggling || !canManageSecurity}
              className={`w-11 h-6 rounded-full relative transition-colors flex-shrink-0 disabled:opacity-50 ${loginOn ? 'bg-teal-500' : 'bg-gray-300 dark:bg-gray-600'}`}
            >
              <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${loginOn ? 'translate-x-5' : 'translate-x-0.5'}`} />
            </button>
          </div>

          <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-600">
            <div className="flex items-center gap-2.5">
              <KeyRound className="w-4 h-4 text-blue-500" />
              <div>
                <p className="text-sm font-medium text-gray-700 dark:text-gray-200">بازیابی رمز با کد یک‌بارمصرف موبایل</p>
                <p className="text-xs text-gray-400">این کانال علاوه بر این کلید، به فعال‌بودن سیاست بازیابی یکپارچه و پیش‌نیازهای Runtime نیاز دارد.</p>
              </div>
            </div>
            <button
              type="button"
              aria-label="فعال‌سازی بازیابی رمز OTP موبایل"
              aria-checked={recoveryOn}
              role="switch"
              onClick={() => { void requestToggle(undefined, !recoveryOn); }}
              disabled={toggling || !canManageSecurity}
              className={`w-11 h-6 rounded-full relative transition-colors flex-shrink-0 disabled:opacity-50 ${recoveryOn ? 'bg-teal-500' : 'bg-gray-300 dark:bg-gray-600'}`}
            >
              <span className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${recoveryOn ? 'translate-x-5' : 'translate-x-0.5'}`} />
            </button>
          </div>

          <div className="grid grid-cols-2 gap-2 p-3 rounded-xl bg-gray-50 dark:bg-gray-700/30 border border-gray-100 dark:border-gray-600">
            <div className="space-y-1.5">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">وضعیت پیش‌نیازها</p>
              <StatusBadge ok={status.provider_active === true} label="سرویس‌دهنده پیامک" />
              <StatusBadge ok={status.login_template_ready === true} label="قالب پیامک ورود" />
              <StatusBadge ok={status.recovery_template_ready === true} label="قالب پیامک بازیابی" />
              <StatusBadge ok={status.origins_set === true} label="نشانی‌های مبدأ مجاز" />
              <StatusBadge ok={status.recovery_secret_configured === true} label="کلید امن بازیابی" />
              <StatusBadge ok={status.recovery_ttl_valid === true} label="مدت اعتبار کد بازیابی" />
            </div>
            <div className="space-y-1.5">
              <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">وضعیت آماده‌باش</p>
              <StatusBadge ok={loginReady} label="ورود OTP آماده" />
              <StatusBadge ok={recoveryReady} label="بازیابی OTP آماده" />
            </div>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <div className="text-center p-2.5 rounded-xl bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800">
              <p className="text-lg font-bold text-green-600 dark:text-green-400">{status.sync_matched ?? 0}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">هماهنگ</p>
            </div>
            <div className="text-center p-2.5 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800">
              <p className="text-lg font-bold text-amber-600 dark:text-amber-400">{(status.sync_profile_only ?? 0) + (status.sync_auth_only ?? 0)}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">ناهماهنگ</p>
            </div>
            <div className="text-center p-2.5 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800">
              <p className="text-lg font-bold text-red-600 dark:text-red-400">{status.sync_mismatched ?? 0}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">تضاد</p>
            </div>
            <div className="text-center p-2.5 rounded-xl bg-orange-50 dark:bg-orange-900/20 border border-orange-100 dark:border-orange-800">
              <p className="text-lg font-bold text-orange-600 dark:text-orange-400">{status.sync_duplicates ?? 0}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">تکراری</p>
            </div>
          </div>

          {status.last_dispatch && (
            <div className="flex items-center gap-2 text-xs text-gray-400 px-1">
              <span>آخرین ارسال:</span>
              <StatusBadge ok={status.last_dispatch.status === 'sent' || status.last_dispatch.status === 'delivered'} label={DISPATCH_STATUS_LABELS[status.last_dispatch.status] || 'وضعیت نامشخص'} />
              <span>•</span>
              <span>{EVENT_TYPE_LABELS[status.last_dispatch.event_type] || 'رویداد نامشخص'}</span>
              {status.last_dispatch.created_at && (
                <>
                  <span>•</span>
                  <span>{new Date(status.last_dispatch.created_at).toLocaleString('fa-IR')}</span>
                </>
              )}
            </div>
          )}
        </div>
      </div>
    </>
  );
}

export { PhoneAuthCard };
