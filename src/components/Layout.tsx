...
function PasswordModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs" dir="rtl">
            ...
            <>
              <div>
                ...
                <input
                  ...
                  className="w-full text-sm px-3 py-2.5 rounded-xl border border-gray-200 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:outline-hidden focus:ring-2 focus:ring-teal-500/40 focus:border-teal-500"
                  placeholder="••••••••"
                />
              </div>
              <div>
...
// ── User Settings Modal (color theme + dark/light + user preferences) ─────────
function SettingsModal({ onClose }: { onClose: () => void }) {
  const { theme, toggleTheme, accent, setAccent } = useTheme();
  const { prefs, updatePrefs } = useUserPreferences();
  const [saving, setSaving] = useState(false);

  const handle = async (patch: Parameters<typeof updatePrefs>[0]) => {
    setSaving(true);
  ...
  const CALENDAR_VIEWS = [
    { key: 'month', label: 'ماهانه', icon: Calendar },
    { key: 'week',  label: 'هفتگی',  icon: CalendarDays },
    { key: 'day',   label: 'روزانه', icon: Clock },
    { key: 'list',  label: 'لیستی',  icon: LayoutList },
  ] as const;

  const LANDING_PAGES = [
    { key: 'calendar', label: 'تقویم',   icon: Calendar },
    { key: 'meetings', label: 'جلسات',   icon: LayoutDashboard },
    { key: 'tasks',    label: 'اقدامات', icon: CheckSquare },
  ] as const;

  const REMINDER_OPTIONS = [5, 10, 15, 30, 60] as const;
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 backdrop-blur-xs" dir="rtl">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all ${theme === 'light' ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300'}`}
              >
              ...
              <button
                ...
                className={`flex flex-col items-center gap-2 p-3 rounded-xl border-2 transition-all ${theme === 'dark' ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20' : 'border-gray-200 dark:border-gray-600 hover:border-gray-300'}`}
              >
          ...
          <div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {ACCENT_COLORS.map(c => (
                ...
                >
                  <span className="w-7 h-7 rounded-full shadow-xs flex items-center justify-center transition-transform group-hover:scale-110" style={{ backgroundColor: c.hex }}>
          ...
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">نمای پیش‌فرض تقویم</p>
            <div className="grid grid-cols-4 gap-1.5">
              {CALENDAR_VIEWS.map(v => {
                const Icon = v.icon;
                const active = prefs.default_calendar_view === v.key;
                return (
                  <button
                    key={v.key}
                    onClick={() => handle({ default_calendar_view: v.key })}
                    className={`flex flex-col items-center gap-1.5 py-2.5 px-1 rounded-xl border-2 transition-all ${active ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400' : 'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-gray-300'}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full py-1.5 px-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-xs focus:outline-hidden focus:ring-2 focus:ring-teal-500"
                      dir="ltr"
                    >
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {LANDING_PAGES.map(p => {
                const Icon = p.icon;
                const active = prefs.default_landing_page === p.key;
                return (
                  <button
                    key={p.key}
                    onClick={() => handle({ default_landing_page: p.key })}
                    className={`flex flex-col items-center gap-1.5 py-2.5 px-1 rounded-xl border-2 transition-all ${active ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400' : 'border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400 hover:border-gray-300'}`}
                  >
                    <Icon className="w-4 h-4" />
          ...
          {/* ── Default reminder */}
          <div>
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-3">یادآوری پیش‌فرض جلسات</p>
            <div className="flex flex-wrap gap-1.5">
              {REMINDER_OPTIONS.map(mins => (
                <button
                  key={mins}
                  onClick={() => handle({ reminder_minutes: mins })}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border-2 transition-all ${prefs.reminder_minutes === mins ? 'border-teal-500 bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400' : 'border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-gray-300'}`}
                >
                  {mins} دقیقه
                </button>
              ))}
          ...
          {/* ── Toggle switches */}
          <div className="space-y-3">
            <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">نمایش و رفتار</p>

            {[
              { key: 'show_past_meetings',      label: 'نمایش جلسات گذشته',         icon: Eye },
        ...
        {/* Footer */}
        <div className="px-5 pb-5 pt-2 shrink-0 border-t border-gray-100 dark:border-gray-700">
          {saving && <p className="text-xs text-teal-500 text-center mb-2">در حال ذخیره...</p>}
                  ...
                  {installPrompt ? (
                    ...
                    !isIOS && (
                      <div className="flex items-center gap-3 p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600">
                        <div className="w-10 h-10 rounded-xl bg-gray-200 dark:bg-gray-600 flex items-center justify-center shrink-0">
...
export function Layout({ children, activePage, onPageChange, isAdmin = false, userPermissions, sparkVisible = false }: LayoutProps) {
  ...
  const TopBar = () => (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 shadow-xs" style={{ height: '52px' }}>
        {/* Left side */}
  ...
  return (
    ...
    >
      ...
      {showInstallBanner && (
        <div className="fixed bottom-0 left-0 right-0 z-[9999] p-3 pointer-events-none" dir="rtl">
          <div className="max-w-sm mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 p-4 flex items-center gap-3 pointer-events-auto">
            ...
            <button onClick={handleInstall} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-colors touch-manipulation shrink-0">نصب</button>
            <button onClick={dismissInstallBanner} className="p-1.5 text-gray-400 hover:text-gray-600 touch-manipulation shrink-0"><X className="w-4 h-4" /></button>
          </div>
        </div>
      )}
      ...
      >
        ...
        <div className="flex items-center justify-center px-3 h-14 border-b border-gray-100 dark:border-gray-700 shrink-0 relative">
          <button
            onClick={() => { const next = !isCollapsed; setIsCollapsed(next); localStorage.setItem('sidebar_collapsed', String(next)); }}
            className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hidden lg:flex shrink-0"
          >
            {isCollapsed
