...
export const VIEW_OPTIONS = [
  ...
  { key: '3-day' as const, label: '۳ روز' },
  { key: '4-day' as const, label: '۴ روز' },
  { key: 'week' as const, label: 'هفته' },
  { key: 'month' as const, label: 'ماه' },
  { key: 'year' as const, label: 'سال' },
  { key: 'schedule' as const, label: 'برنامه' },
];

export type CalendarViewMode = typeof VIEW_OPTIONS[number]['key'];

export function isMultiDayView(view: string): boolean {
  return view === '3-day' || view === '4-day' || view === 'work-week' || view === 'week';
}

export function viewDayCount(view: string): number {
  if (view === '3-day') return 3;
  if (view === '4-day') return 4;
  if (view === 'work-week') return 5;
  return 7;
}
