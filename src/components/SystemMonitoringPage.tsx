...
import { CalendarDays, MessageSquare, SquareCheck as CheckSquare, Activity, Loader as Loader2 } from 'lucide-react';
...
import { DailyReportMonitor } from './SystemMonitoring/DailyReportMonitor';
import { supabase } from '../lib/supabase';

type Section = 'meetings' | 'chat' | 'tasks' | 'daily-report';
...
const TABS: { key: Section; label: string; icon: React.ElementType; color: string; desc: string }[] = [
  ...
  { key: 'daily-report', label: 'ارسال جلسات مدیریتی', icon: Activity, color: 'text-violet-600 dark:text-violet-400 bg-violet-100 dark:bg-violet-900/30', desc: 'تست و گزارش Scheduler و ارسال روزانه' },
];
...
export function SystemMonitoringPage() {
  ...
  return (
    <div className="space-y-5" dir="rtl">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
        {TABS.map(tab => {
          ...
          const borderColor = active
            ? (tab.key === 'meetings' ? '#3b82f6' : tab.key === 'chat' ? '#14b8a6' : tab.key === 'tasks' ? '#f59e0b' : '#7c3aed')
            : undefined;
          return (
      ...
      {section === 'daily-report' && <DailyReportMonitor />}
    </div>
  );
}
