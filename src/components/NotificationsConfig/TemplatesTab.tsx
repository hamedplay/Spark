        ...
        <div className="p-5 space-y-4">
          <div className="flex flex-wrap gap-1.5">
            ...
            <span className={`text-xs px-2.5 py-1 rounded-full ${AUDIENCE_COLORS[template.audience] || AUDIENCE_COLORS.all}`}>
              {AUDIENCES.find(a => a.key === template.audience)?.label || audienceLabel[template.audience] || template.audience}
            </span>
          </div>
...
export function TemplatesTab() {
  ...
  const catalogAudienceOrder = AUDIENCES.map(a => a.key);
  const audienceOrder = Array.from(new Set([...catalogAudienceOrder, ...filtered.map(t => t.audience)]));
      ...
      <div className="flex flex-wrap gap-2">
        {AUDIENCES.map(a => (
          <button key={a.key} onClick={() => setFilterAudience(filterAudience === a.key ? 'all' : a.key)}
            className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full border font-medium transition-all ${filterAudience === a.key || filterAudience === 'all' ? (AUDIENCE_COLORS[a.key] || AUDIENCE_COLORS.all) + ' border-transparent' : 'border-gray-200 dark:border-gray-600 text-gray-400'}`}>
        ...
        {Object.entries(grouped).map(([cat, audienceMap]) => (
          <div key={cat} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            {audienceOrder.filter(aud => audienceMap[aud]?.length).map(aud => (
              <div key={aud}>
                <div className={`flex items-center gap-2 px-4 py-2 text-xs font-medium border-b border-gray-50 dark:border-gray-700/50 ${AUDIENCE_COLORS[aud] || AUDIENCE_COLORS.all}`}>
                  <Users className="w-3 h-3" />
                  {AUDIENCES.find(a => a.key === aud)?.label || audienceLabel[aud] || aud}
