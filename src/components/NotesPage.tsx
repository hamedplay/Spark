...
import type { Note, NoteStatusFilter } from './Notes/types';
...
import { CreateNoteForm, CreateNoteButton } from './Notes/CreateNoteForm';
import { NotesToolbar } from './Notes/NotesToolbar';
import { NoteCard } from './Notes/NoteCard';
import { VoiceNoteFab } from './Notes/VoiceNoteFab';
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredNotes.map((note, index) => (
            <NoteCard
              ...
              shareMenuNoteId={shareMenuNoteId}
              shareMenuRef={shareMenuRef}
              canEdit={canEdit}
              canDelete={canDelete}
              onEditNote={handleEditNote}
              onEditChange={setEditingNote}
              onSaveEdit={handleSaveEdit}
              onCancelEdit={() => { setEditingNoteId(null); setEditingNote(null); }}
              onToggleExpand={() => setExpandedNoteId(expandedNoteId === note.id ? null : note.id)}
              onToggleShareMenu={() => setShareMenuNoteId(v => v === note.id ? null : note.id)}
              onShareImage={handleShareImage}
              onShareText={handleShareText}
              onAssign={(n) => { setAssignNote(n); setAssignSearch(''); }}
