...
| تنظیمات مقصد | IP روی مقصد انتخاب می‌شود؛ تنظیمات سرور سازنده و کلیدهای خصوصی TLS در باندل جدید کپی نمی‌شوند. |

خروجی فقط پس از موفقیت کنترل کامل بودن، checksum همهٔ فایل‌ها، بررسی آرشیوها و آزمون آفلاین ساخته می‌شود. نتیجهٔ آزمون APT/npm/frontend در `npm/offline-proof.env` ثبت می‌شود. این آزمون یک نصب کامل ۲۱مرحله‌ای روی VM نیست؛ سلامت سرویس‌های در حال اجرا، WAF، پورت‌ها و ارتباط با ارائه‌دهندگان خارجی باید روی مقصد بررسی شود.
...
این خروجی بستهٔ نصب است و شامل دیتابیس زنده، کاربران، فایل‌های آپلودشده یا داده‌های volumeها نیست. مرحلهٔ ۲۱ در صورت نبود ساختار برنامه، مسیر بکاپ محلی و تأیید بازیابی را در همان جریان نصب دریافت می‌کند؛ سپس با بکاپ ایمنی، بازیابی، راه‌اندازی سرویس‌ها و اعتبارسنجی ادامه می‌دهد. ساختار کامل دیتابیس برنامه از هیچ ساخته نمی‌شود؛ بکاپ سازگار باید همراه استقرار باشد. وجود فایل migration داخل Git bundle به معنی اعمال آن روی دیتابیس نیست. ابزارهای آنلاینِ مستقل مانند دریافت `supabase@latest` با npx جزو زنجیرهٔ نصب آفلاین ۲۱مرحله‌ای نیستند.

گزینه‌های **Run complete offline installation** و **Run one offline install step** دقیقاً از یک آماده‌سازی و اجرای مشترک استفاده می‌کنند: مراحل ۱ تا ۱۷ و ۱۹ تا ۲۲. مرحلهٔ فایروال حذف شده و DNS و گواهی، پیش‌نیاز نصب نیستند.

- هر دو مسیر قفل مشترک دارند تا دو نصب هم‌زمان کش و تنظیمات را تغییر ندهند.
- تصویر حذف‌شده یا با شناسهٔ نامنطبق، پس از اعتبارسنجی باندل از آرشیو محلی بازیابی و دوباره بررسی می‌شود؛ هیچ pull اینترنتی انجام نمی‌شود.
- مرحلهٔ ۱۰ کش را پیش از روشن‌شدن Functions با تعداد فایل و checksum بررسی می‌کند. مراحل ۱۰ و ۲۰ پس از راه‌اندازی Functions حداکثر ۹۰ ثانیه منتظر پاسخ واقعی HTTP 204 می‌مانند؛ خطای دائمی موفق اعلام نمی‌شود.
- اجرای مجدد هر مرحله، سابقهٔ موفقیت آن مرحله و مراحل بعدی را پاک می‌کند؛ مرحلهٔ ۲۲ نمی‌تواند از تأیید قدیمی مرحلهٔ ۲۱ استفاده کند.
- تمام این رفتارها داخل نصب‌کننده‌اند و به کپی دستی فایل اصلاحیه یا اجرای دستور ترمیم جداگانه نیاز ندارند. باندل باید با سورس اصلاح‌شده ساخته شود؛ باندل قدیمی کد جدید را خودبه‌خود دریافت نمی‌کند.

محدودهٔ حالت HTTP روی IP: راه‌اندازی سرویس‌ها و دیتابیس به DNS و گواهی نیاز ندارد، اما این به معنی کارکرد تمام قابلیت‌های مرورگر روی HTTP نیست. دسترسی دوربین/میکروفون با `getUserMedia` و متد `crypto.randomUUID` به secure context نیاز دارند؛ برنامه در تماس، تخته‌سفید، برخی فرم‌ها و MFA از این APIها استفاده می‌کند. نصب‌کننده نمی‌تواند محدودیت مرورگر کاربران را با تنظیم Nginx حذف کند. برای استفادهٔ کامل باید دسترسی امنِ مورد اعتماد مرورگر فراهم شود؛ این نیاز مستقل از اتصال اینترنت است. منابع: [getUserMedia](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia)، [randomUUID](https://developer.mozilla.org/en-US/docs/Web/API/Crypto/randomUUID).
