-- Production compatibility repair for user-selectable canonical MFA methods.
-- This asset is intentionally self-contained because deployment repairs must not
-- depend on historical migration files remaining present in the repository.

CREATE TABLE IF NOT EXISTS public.mfa_switch_intents (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  session_id uuid NOT NULL REFERENCES auth.sessions(id) ON DELETE CASCADE,
  from_method text NOT NULL,
  to_method text NOT NULL,
  status text NOT NULL DEFAULT 'pending_stepup',
  current_method_verified_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now(),
  expires_at timestamptz NOT NULL DEFAULT (now() + interval '10 minutes'),
  completed_at timestamptz
);

ALTER TABLE public.mfa_switch_intents
  ADD COLUMN IF NOT EXISTS current_method_verified_at timestamptz,
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conrelid = 'public.mfa_switch_intents'::regclass
      AND conname = 'mfa_switch_intents_from_method_check'
  ) THEN
    ALTER TABLE public.mfa_switch_intents
      ADD CONSTRAINT mfa_switch_intents_from_method_check
      CHECK (from_method = ANY (ARRAY['totp'::text, 'sms'::text]));
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'public.mfa_switch_intents'::regclass
      AND conname = 'mfa_switch_intents_check'
  ) THEN
    ALTER TABLE public.mfa_switch_intents
      ADD CONSTRAINT mfa_switch_intents_check
      CHECK (to_method = ANY (ARRAY['totp'::text, 'sms'::text]) AND to_method <> from_method);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conrelid = 'public.mfa_switch_intents'::regclass
      AND conname = 'mfa_switch_intents_status_check'
  ) THEN
    ALTER TABLE public.mfa_switch_intents
      ADD CONSTRAINT mfa_switch_intents_status_check
      CHECK (status = ANY (ARRAY['pending_stepup'::text, 'ready'::text, 'pending_enrollment'::text, 'completed'::text, 'expired'::text, 'cancelled'::text]));
  END IF;
END
$repair$;

CREATE UNIQUE INDEX IF NOT EXISTS mfa_switch_one_open_intent_per_user
  ON public.mfa_switch_intents(user_id)
  WHERE status = ANY (ARRAY['pending_stepup'::text, 'ready'::text, 'pending_enrollment'::text]);

ALTER TABLE public.mfa_switch_intents ENABLE ROW LEVEL SECURITY;
...
REVOKE ALL ON TABLE public.mfa_switch_intents FROM PUBLIC;
REVOKE ALL ON TABLE public.mfa_switch_intents FROM anon;
REVOKE ALL ON TABLE public.mfa_switch_intents FROM authenticated;
...
BEGIN
  ...
  INTO v_method, v_phone, v_is_security_admin, v_user_requires_mfa
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(p.mfa_enrollment_required, false)
  INTO
    v_from, v_epoch, v_account_status, v_is_active,
    v_is_security_admin, v_user_requires_mfa
  FROM public.profiles p
  WHERE p.user_id = v_uid
  FOR UPDATE;
