...
test_frontend_build_security() {
  require_file "${SPARK_ROOT}/dist/index.html" || return 1
  require_file "${SPARK_ROOT}/dist/sw.js" || return 1
  require_file "${SPARK_ROOT}/dist/pwa-bootstrap.js" || return 1

  local key value
  for key in SERVICE_ROLE_KEY JWT_SECRET POSTGRES_PASSWORD DASHBOARD_PASSWORD; do
    value="$(env_get "${SUPABASE_ROOT}/.env" "$key")"
    [[ -n "$value" ]] || continue
    if grep -R -F -q -- "$value" "${SPARK_ROOT}/dist"; then
      echo "Private secret leaked into frontend build: ${key}" >>"$CURRENT_LOG"
      return 1
    fi
  done

  if grep -R -F -q -- 'icpgvfadixevdjtkllap.supabase.co' "${SPARK_ROOT}/dist"; then
    echo "Dead legacy Supabase origin is still present in frontend build" >>"$CURRENT_LOG"
    return 1
  fi
}
