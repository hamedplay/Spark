...
### Phase 4C — User-preferences data boundary and Auth-realtime lifecycle macro pass ✅

Ten completed items:

1. User Preferences domain model and defaults — `src/features/user-preferences/types/userPreferences.ts` created with `UserPreferences`, `UserPreferencesPatch`, `DefaultCalendarView`, `DefaultLandingPage`, `UserPreferenceTheme`, `UserPreferencesRow`, and `DEFAULT_USER_PREFERENCES`; exact legacy defaults preserved; no freeze, no validation, no field renames
2. Pure row mapper and merge behavior — `src/features/user-preferences/mappers/mapUserPreferencesRow.ts` created with `mapUserPreferencesRow` and `mergeUserPreferences`; field-level assertions plus nullish fallback preserved exactly; false, zero, empty strings, invalid enum strings, and nullable work-hour values preserved; merge is shallow spread without mutation
3. Eight mapping characterization tests — `tests/user-preferences/userPreferencesMapping.test.ts` created with eight tests covering exact defaults, complete row mapping, nullish fallback, false booleans, zero reminder, nullable/empty work hours, asserted enum strings, and merge without mutation
4. User Preferences repository — `src/features/user-preferences/repositories/userPreferencesRepository.ts` created with `fetchUserPreferencesRow` and `upsertUserPreferences`; fetch uses `select('*')`, `.eq('user_id', userId)`, `.maybeSingle()`, ignores errors, returns null for missing data; upsert uses `onConflict: 'user_id'`, ignores errors, no rollback, no `.select()`; `updatedAt` generated by Provider
5. Auth User-ID subscription adapter — `subscribeToAuthUserIdChanges` added to `src/features/auth/services/authOperations.ts` with `AuthUserIdChangeListener` type; calls listener on every Auth event with user ID or null; no event filtering, no async callback, no logging; exported through `src/features/auth/index.ts`
6. Lifecycle controller with six tests — `src/features/user-preferences/services/userPreferencesLifecycle.ts` created with `startUserPreferencesLifecycle`; generation token invalidates stale initial and previous-user loads; cleanup marks disposed, increments generation, unsubscribes once; `tests/user-preferences/userPreferencesLifecycle.test.ts` created with six tests covering initial user load, no initial user, sign-out callback, later Auth user, stale load invalidation, and cleanup ignoring late work
7. Feature-local Provider and public API — `src/features/user-preferences/context/UserPreferencesContext.tsx` created with `UserPreferencesProvider`, `useUserPreferences`, and `UserPreferencesContextValue`; Provider Effect calls lifecycle controller and returns synchronous cleanup; preferences loaded through repository and mapper; localStorage writes for theme and accent_color preserved after row load; sign-out resets to defaults without clearing localStorage; `updatePrefs` optimistic with `mergeUserPreferences`, no rollback, callback dependencies preserved; `src/features/user-preferences/index.ts` exports Provider, hook, defaults, and types only
8. Migration of Main, Theme, and Settings consumers — `src/main.tsx`, `src/context/ThemeContext.tsx`, `src/app/layout/components/UserSettingsModal.tsx` updated to import through feature public API; `src/App.tsx`, `src/app/navigation/useNavigation.ts`, `src/components/CalendarPage.tsx` also migrated; Provider ordering preserved exactly
9. Deletion of legacy Context file — `src/context/UserPreferencesContext.tsx` deleted after zero importers confirmed; no compatibility bridge
10. 49 passing tests, build, and scoped lint — 6+6+6+5+8+4+8+6 = 49 tests pass; build passes; scoped lint 0 errors, 5 warnings (4 pre-existing in ThemeContext, 1 expected react-refresh in new Provider)

Lifecycle correction:
Late initial or previous-user preference loads can no
longer overwrite the active user's preferences.

Remaining legacy risks:
Preference Upsert errors remain ignored and optimistic
state is not rolled back.

Rapid overlapping updatePrefs calls still merge from the
callback's captured preferences snapshot.

Database enum-like preference values remain trusted by
TypeScript assertions and are not runtime validated.

User preferences do not yet subscribe to Postgres
Realtime changes from another browser or device.

ThemeProvider's existing preference-synchronization
behavior was intentionally left unchanged.

### Phase 4D — Notification consumer data boundary and Realtime lifecycle macro pass ✅

Twelve completed items:

1. Notification domain model — `src/features/notifications/types/appNotification.ts` created with `AppNotification` and `NotificationGroup`; exact fields preserved from legacy component; no producer payload fields added; no snake_case conversion
2. Pure collection and Navigation behavior — `src/features/notifications/models/notificationCollection.ts` created with `countUnreadNotifications`, `prependIncomingNotification`, `replaceUpdatedNotification`, `markNotificationReadLocally`, `markAllNotificationsReadLocally`, `formatNotificationTimeAgo`, `groupNotificationsByDate`; all algorithms preserved exactly including 50-row limit, deduplication by reference identity, consecutive grouping, Persian relative time, and try/catch empty-string fallback; `src/features/notifications/navigation/notificationNavigation.ts` created with `NOTIFICATION_PAGE_MAP`, `resolveNotificationClickPage` (action_url → template_event_type → type precedence), and `resolveNotificationToastPage` (action_url-only)
3. Twelve model characterization tests — `tests/notifications/notificationModel.test.ts` created with twelve tests covering click precedence, toast-only navigation, full map entries, unread count, deduplication, 50-row limit, Realtime update replacement, mark-one, mark-all, relative time boundaries, today/yesterday grouping, and consecutive grouping
4. Notification repository — `src/features/notifications/repositories/notificationRepository.ts` created with `fetchUserNotifications`, `markNotificationAsRead`, `markAllUserNotificationsAsRead`; fetch preserves `select('*')`, `.eq('user_id')`, `.order('created_at', { ascending: false })`, `.limit(50)`, error throwing; mark-one preserves conditional user filter; mark-all preserves `.eq('read', false)`
5. Realtime adapter — `src/features/notifications/services/notificationRealtime.ts` created with `subscribeToUserNotificationChanges`; channel name `notifications-bell-${userId}-${Date.now()}`; INSERT and UPDATE handlers on same channel with `user_id=eq.${userId}` filter; `.subscribe()` called once; cleanup via `supabase.removeChannel(channel)`; no DELETE handling
6. Bell lifecycle controller with six tests — `src/features/notifications/services/notificationBellLifecycle.ts` created with `startNotificationBellLifecycle`; startup order preserved (loading true → async fetch → synchronous Realtime subscription); disposed flag ignores late fetch and Realtime callbacks; cleanup guarded to unsubscribe exactly once; `tests/notifications/notificationBellLifecycle.test.ts` created with six tests covering startup order, successful load, error reporting, Realtime forwarding, single unsubscribe, and post-cleanup ignoring
7. Notification Bell hook — `src/features/notifications/hooks/useNotificationBell.ts` created with `useNotificationBell`; owns current-user ID state, mount-only Auth lookup with disposed flag, Notification lifecycle startup, notification list, unread count, optimistic mark-one with rollback, optimistic mark-all with rollback, incoming Toast/Browser Notification for every INSERT, and `handleNotificationClick` returning `Promise<boolean>` (true = navigation occurred, false = no mapped target or no onNavigate)
8. Incoming Toast/Browser alert service — `src/features/notifications/services/showIncomingNotification.tsx` created with `showIncomingNotification`; 6000ms Toast duration, sender/avatar logic, type background and icon, chat sender-name rendering, Persian navigation hint, dismiss button, click-to-dismiss-then-navigate, Browser Notification only when window exists and permission granted and document not visible, Browser icon `/logo_spark.png`, Browser click focuses window and navigates only when target exists
9. Presentation extraction — `src/features/notifications/components/NotificationVisuals.tsx` created with `NotificationTypeIcon`, `getNotificationTypeBackground`, `NotificationAvatar`; `src/features/notifications/components/NotificationPanelContent.tsx` created with panel header, loading state, empty state, grouped rows, footer, and row rendering; `src/features/notifications/components/NotificationBell.tsx` created owning only `showPanel`, outside-click ref, date grouping, mobile overlay, mobile/desktop panel shells, Bell button, ring animation, and hook orchestration
10. LayoutTopBar migration — import path changed from `../../../components/NotificationBell` to `../../../features/notifications`; no other Top Bar behavior or JSX modified
11. Legacy component deletion — `src/components/NotificationBell.tsx` deleted after zero importers confirmed; no compatibility bridge
12. 67 passing tests, build, and scoped lint — 6+6+6+5+8+4+8+6+12+6 = 67 tests pass; build passes; scoped lint 0 errors, 1 expected react-refresh warning in NotificationVisuals

Lifecycle guarantee:
Notification Realtime callbacks and initial-load results
are ignored after lifecycle cleanup.

Remaining legacy risks:
The Notification Bell performs only a mount-time Auth
lookup and does not follow later Auth user changes.

An initial fetch can still race with an early Realtime
insert and overwrite the locally inserted row.

Mark-read rollback uses captured Notification snapshots
and can overwrite concurrent Realtime changes.

Notification database rows are trusted through type
assertions without runtime validation.

Fetch failures remain console-only.

The Notification producer and delivery implementation in
src/lib/notifications.ts remains a separate legacy
boundary.

### Phase 4E — Notification producer, template cache, and delivery boundary macro pass ✅

Fifteen completed items:

1. Producer contracts — `src/features/notifications/types/notificationProducer.ts` created with `NotifyChannels`, `NotifyPayload`, `SmsDispatchResult`, `NotificationTemplateRow`, `NotificationTemplate`, `SmsTemplateRow`, `NotificationChannelSelection`; all fields preserved from legacy file; no narrowing of `category`, `eventType`, or `audience`
2. Pure Producer template/channel model — `src/features/notifications/models/notificationProducerModel.ts` created with `buildNotificationTemplateKey`, `resolveNotificationAudience`, `buildNotificationTemplateMap`, `buildSmsTemplateMap`, `resolveNotificationTemplate`, `resolveSmsTemplateBody`, `resolveNotificationChannels`, `buildBaleNotificationText`, `isDuplicateNotificationRpcResult`; all algorithms preserved exactly including first-wins for Notification templates, last-wins for SMS templates, `||` audience fallback, `!== false` channel defaults, and `created === false` duplicate detection
3. Eight Producer-model tests — `tests/notifications/notificationProducerModel.test.ts` created with eight tests covering template key, audience fallback, first-wins, last-wins, exact-then-all precedence, channel defaults, Bale text, and duplicate RPC detection
4. Notification and SMS template repository — `src/features/notifications/repositories/notificationTemplateRepository.ts` created with `fetchActiveNotificationTemplateRows` and `fetchActiveSmsTemplateRows`; Notification query preserves `select`, `.eq('is_active', true)`, `.order('updated_at', { ascending: false })`, no limit, error ignored, `data || []`; SMS query preserves no order, no limit, error ignored
5. Specific five-minute cache service — `src/features/notifications/services/notificationTemplateCache.ts` created with `createNotificationTemplateCache` factory, `NOTIFICATION_TEMPLATE_CACHE_TTL_MS`, independent Notification and SMS cache state, strict `<` TTL comparison, loaded-at set after fetch, no in-flight deduplication, invalidate sets both maps to null; `src/features/notifications/services/notificationTemplateDefaults.ts` created with default cache using real repository functions and `Date.now`, exporting `getNotificationTemplates`, `getSmsTemplates`, `invalidateTemplateCache`
6. Five cache characterization tests — `tests/notifications/notificationTemplateCache.test.ts` created with five tests covering TTL reuse, exact-boundary reload, cache independence, invalidation preserving mapping rules, and concurrent-miss non-deduplication
7. In-app Notification repository — `src/features/notifications/repositories/inAppNotificationRepository.ts` created with `createInAppNotification`; preserves exact RPC payload, `p_type` cast, error-to-failed mapping, duplicate detection, no `.select()`, no retries, no logging
8. SMS delivery gateway and failure logging — `src/features/notifications/gateways/smsNotificationGateway.ts` created with `dispatchSmsNotification`; preserves Edge Function invocation, `functionError` throw, result normalization, catch-unknown logging to `sms_dispatch_logs` with `CLIENT_INVOKE_ERROR` prefix, insert error ignored, structured failure return
9. Bale delivery gateway — `src/features/notifications/gateways/baleNotificationGateway.ts` created with `dispatchBaleNotification`; preserves `fetch` call, environment-variable lookup, session token precedence, exact URL and headers, response ignored, all failures swallowed, no `supabase.functions.invoke`
10. Producer orchestration service — `src/features/notifications/services/notificationProducerOrchestration.ts` created with `fillPlaceholders` and `produceNotificationWithDependencies`; `src/features/notifications/services/notificationProducer.ts` created with `insertNotification` wiring real dependencies; preserves complete operation order, template resolution, Development diagnostics, title/message selection, in-app early returns (failed and duplicate), SMS lazy-load and dispatch, Bale fire-and-forget, and CHANNEL_DISABLED default
11. Seven Producer orchestration tests — `tests/notifications/notificationProducer.test.ts` created with seven tests covering in-app→SMS→Bale ordering, RPC_ERROR skip, duplicate skip, in-app-disabled continuation, SMS-disabled with Bale, SMS fallback precedence, and all-channel-disabled template loading
12. Compatibility facade — `src/lib/notifications.ts` converted to re-exports from new feature internals; preserves `insertNotification`, `fillPlaceholders`, `getSmsTemplates`, `invalidateTemplateCache`, `NotifyChannels`, `NotifyPayload`, `SmsDispatchResult`; no Supabase import, cache state, Date usage, Template Catalog import, fetch call, Edge Function invocation, RPC call, or orchestration code
13. Notifications public Producer API — `src/features/notifications/index.ts` updated to export `NotificationBell`, `AppNotification`, `insertNotification`, `invalidateTemplateCache`, `NotifyChannels`, `NotifyPayload`, `SmsDispatchResult`; does not export `produceNotificationWithDependencies`, `getNotificationTemplates`, `getSmsTemplates`, `fillPlaceholders`, repositories, gateways, cache factory, or model helpers
14. NotificationVisuals warning closure — `src/features/notifications/models/notificationVisualStyle.ts` created with `getNotificationTypeBackground`; `NotificationVisuals.tsx` now exports only `NotificationTypeIcon` and `NotificationAvatar`; `NotificationPanelContent.tsx` and `showIncomingNotification.tsx` updated to import from the model file; react-refresh warning removed
15. 87 passing tests, build, and zero-warning scoped lint — 6+6+6+5+8+4+8+6+12+6+8+5+7 = 87 tests pass; build passes; scoped lint 0 errors, 0 warnings

Preserved legacy behaviors:
Notification template duplicate keys remain first-wins
because the Query is ordered newest-first.

SMS template duplicate keys remain last-wins because the
Query has no explicit ordering.

Template cache misses are not deduplicated while a fetch
is in flight.

Notification and SMS template Query errors remain
ignored.

An in-app RPC error or duplicate event key prevents SMS
and Bale dispatch.

Bale delivery remains fire-and-forget and swallows all
failures.

SMS client-invocation failures still return a structured
failure and attempt a secondary log insert.

Remaining risks:
Template cache invalidation remains manual and
session-local.

Template rows and Edge Function result objects are
trusted through TypeScript assertions without runtime
validation.

SMS template selection remains nondeterministic when the
database contains duplicate active rows because its
Query has no ordering.

Bale fetch responses are not inspected.

Existing callers still use the legacy
src/lib/notifications.ts compatibility facade.

The Notification Bell still has the consumer-side legacy
risks recorded in Phase 4D.

### Phase 4 — Split oversized feature files (in progress)

Next work item: Phase 4F — Permissions Provider data boundary and lifecycle macro batch
