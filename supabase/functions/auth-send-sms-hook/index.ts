...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Final delivery gate: direct /auth/v1/otp calls have no short-lived,
    // server-issued grant and therefore cannot trigger an SMS.
    const grantPepper = Deno.env.get("PHONE_RATE_LIMIT_PEPPER") ?? "";
    if (grantPepper.length < 32) {
      await supabase.rpc("fail_auth_hook_event", { p_webhook_id: webhookId, p_error_code: "GRANT_CONFIG_ERROR" });
      return errorResponse(503, "SMS delivery unavailable");
    }
    const phoneHash = await hmacSha256Hex(grantPepper, `native-auth-sms|${normalizedPhone}`);
    const { data: grantConsumed, error: grantError } = await supabase.rpc(
      "consume_native_auth_sms_send_grant_v1",
      { p_phone_hash: phoneHash, p_purpose: "auth_sms" },
    );
    if (grantError || grantConsumed !== true) {
      await supabase.rpc("fail_auth_hook_event", { p_webhook_id: webhookId, p_error_code: "SMS_GRANT_REQUIRED" });
      return errorResponse(403, "SMS delivery not authorized");
    }
...
async function hmacSha256Hex(key: string, value: string): Promise<string> {
  ...
  const cryptoKey = await crypto.subtle.importKey(
    "raw", bytes.encode(key), { name: "HMAC", hash: "SHA-256" }, false, ["sign"],
  );
  const signature = await crypto.subtle.sign("HMAC", cryptoKey, bytes.encode(value));
