...
type ModeratorChatAction = "send" | "edit" | "delete";
...
Deno.serve(async (req: Request) => {
  ...
  if (!["send", "edit", "delete"].includes(body.action)) {
  ...
  if (authorizationError || !authorization?.ok) {
    return reply(403, {
      error: String(
        ...
        || "MODERATOR_CHAT_ACTION_FORBIDDEN",
      ).toUpperCase(),
    });
  }
  ...
  if (error || !data?.ok) {
    return reply(409, {
      error: String(
        data?.reason || error?.message || "MODERATOR_CHAT_ACTION_FAILED",
      ).toUpperCase(),
    });
  }
