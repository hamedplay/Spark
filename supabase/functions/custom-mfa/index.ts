...
async function mirrorToBale(admin: ReturnType<typeof adminClient>, userId: string, otp: string, purpose: "mfa_login" | "mfa_enrollment", eventRef: string) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (mode === "enroll_sms") {
      ...
      const otp = randomOtp();
      // The RPC owns the UUID.  Replace the temporary hash with one bound to
      // that persisted ID before any code is delivered.
      const otpHash = await domainBoundOtpHash("pending", caller.user.id, caller.sessionId, "sms", phoneHash, otp);
      const { data: challenge, error: challengeError } = await admin.rpc("create_sms_mfa_challenge_v3", {
        p_user_id: caller.user.id, p_session_id: caller.sessionId, p_otp_hash: otpHash, p_target_hash: phoneHash, p_purpose: "enrollment",
      });
      if (challengeError || !challenge?.ok) {
        return json({ ok: false, error: "CHALLENGE_CREATE_FAILED" }, 503);
      }
      const actualOtpHash = await domainBoundOtpHash(challenge.challenge_id, caller.user.id, caller.sessionId, "sms", phoneHash, otp);
      ...
      if (hashError || !await sendSmsOtp(caller.user.id, caller.user.phone, otp)) {
    ...
    if (mode === "create") {
      ...
      if (factorType !== "sms") return json({ ok: false, error: "FACTOR_UNAVAILABLE" }, 409);
      ...
      const { data: challenge, error } = await admin.rpc("create_sms_mfa_challenge_v3", {
        p_user_id: caller.user.id, p_session_id: caller.sessionId,
        p_otp_hash: otpHash, p_target_hash: targetHash, p_purpose: "login",
      });
      ...
      if (!await sendSmsOtp(caller.user.id, caller.user.phone!, otp)) {
        await admin.from("custom_mfa_challenges").update({ status: "expired" }).eq("id", actualChallengeId);
        return json({ ok: false, error: "TRANSPORT_UNAVAILABLE" }, 503);
      }
      EdgeRuntime.waitUntil(mirrorToBale(admin, caller.user.id, otp, "mfa_login", actualChallengeId));
    ...
    if (mode === "resend") {
      ...
      // Resend is deliberately SMS-only.  Bale mirrors the same code and is
      // never treated as an independent factor or challenge target.
      ...
      if (challengeRow.factor_type !== "sms") return json({ ok: false, error: "FACTOR_UNAVAILABLE" }, 409);
      if (isPhoneOtpPrimary(caller.token)) return json({ ok: false, error: "FACTOR_INDEPENDENCE_REQUIRED" }, 409);
      if (!caller.user.phone || !caller.user.phone_confirmed_at) return json({ ok: false, error: "PHONE_NOT_CONFIRMED" }, 400);
