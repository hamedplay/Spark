...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (scheduled && !dryRun) {
      try {
        await supabase.from("daily_report_scheduler_heartbeat").upsert({
          singleton_id: 1,
          last_seen_at: new Date().toISOString(),
          last_tehran_date: tehranNow.date,
          last_tehran_time: tehranNow.time,
          configured_time: config.send_time,
          source: "systemd",
          updated_at: new Date().toISOString(),
        }, { onConflict: "singleton_id" });
      } catch { /* heartbeat must never block report delivery */ }
    }
