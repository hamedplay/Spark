...
export type BaleAuthCodePurpose = "phone_login" | "phone_password_recovery" | "mfa_login" | "mfa_enrollment";
...
function purposeConfigKey(purpose: BaleAuthCodePurpose): string {
  if (purpose === "phone_login") return "phone_login_bale_otp_enabled";
  if (purpose === "phone_password_recovery") return "phone_password_recovery_bale_otp_enabled";
  return purpose === "mfa_login" ? "mfa_login_bale_otp_enabled" : "mfa_enrollment_bale_otp_enabled";
}
...
export async function sendBaleAuthCode(options: SendBaleAuthCodeOptions): Promise<BaleAuthCodeResult> {
  ...
  try {
    ...
    // 6. Read template
    const templateEventType = purpose === "phone_login" ? "login_otp_bale"
      : purpose === "phone_password_recovery" ? "password_reset_otp_bale"
      : purpose === "mfa_login" ? "mfa_otp_bale" : "mfa_enrollment_otp_bale";
