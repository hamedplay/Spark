...
type ChatAction = "send" | "edit" | "delete" | "react";
...
Deno.serve(async (req: Request) => {
  ...
  if (!["send", "edit", "delete", "react"].includes(body.action)) {
  ...
  const { data, error } = await service.rpc("apply_conference_chat_action", {
    p_room_id: body.roomId,
    ...
    p_image_path: body.imagePath ?? null,
  });

  if (error || !data?.ok) {
    const reason = String(
      data?.reason || error?.message || "CHAT_ACTION_FAILED",
    ).toUpperCase();
