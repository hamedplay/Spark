...
type TimerAction = "start" | "extend" | "pause" | "resume" | "stop";
...
function shouldMute(status: string): boolean {
  return status === "PAUSED" || status === "EXPIRED" || status === "COMPLETED";
}
...
Deno.serve(async (req: Request) => {
  ...
  if (!["start", "extend", "pause", "resume", "stop"].includes(body.action)) {
  ...
  if (accessError || !accessState || accessState.access_level !== "FULL") {
  ...
  const { data: authorization, error: authorizationError } = await userClient.rpc(
    "authorize_conference_speaker_timer_action",
    {
  ...
  if (authorizationError || !authorization?.ok) {
    return reply(403, {
      error: String(
        authorization?.reason || authorizationError?.message || "TIMER_ACTION_FORBIDDEN",
      ).toUpperCase(),
    });
  }

  const { data, error } = await service.rpc(
    "apply_livekit_conference_speaker_timer_action",
    {
      p_room_id: body.roomId,
      p_target_user_id: body.targetUserId,
      p_action: body.action,
  ...
  if (error || !data?.ok) {
    return reply(409, {
      error: String(data?.reason || error?.message || "TIMER_ACTION_FAILED").toUpperCase(),
    });
  }

  if (body.action === "extend") {
    return reply(200, { ok: true, session: data.session, serverTime: data.server_time });
  }
  ...
  if (!sessionId || !policy.ok) return reply(500, { error: "TIMER_POLICY_MISSING" });
  ...
  try {
    ...
    console.error("conference-speaker-timer-control: LiveKit sync failed", syncError);
    await service.rpc("complete_conference_speaker_enforcement", {
      p_session_id: sessionId,
      p_success: false,
      p_error: syncError instanceof Error ? syncError.message : "LIVEKIT_SYNC_FAILED",
      p_runtime_updated: false,
    });
    return reply(502, {
      error: "LIVEKIT_TIMER_SYNC_PENDING",
      session: data.session,
      serverTime: data.server_time,
    });
  }
