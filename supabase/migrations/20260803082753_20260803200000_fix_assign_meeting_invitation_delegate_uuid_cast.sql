/*
# Fix assign_meeting_invitation_delegate: 22P02 invalid input syntax for type uuid: ""
#
# Root cause: COALESCE(v_meeting.calendar_id, '') in jsonb_build_object metadata.
# meetings.calendar_id is uuid, '' is text. PostgreSQL resolves COALESCE return type
# at parse time by finding common type — tries to cast '' to uuid → 22P02.
#
# Fix: cast calendar_id to text before COALESCE: COALESCE(v_meeting.calendar_id::text, '')
# Also add 'calendar_id' as raw uuid to metadata for clarity.
#
# All other COALESCE patterns verified safe:
#   - subject, request_date, start_time, end_time, location are all text → COALESCE(x, '') OK
#   - participant_user_ids is uuid[] → COALESCE(x, '{}'::uuid[]) OK
#   - is_hidden is boolean → COALESCE(x, false) OK
#   - updated_at/created_at are timestamptz → COALESCE(x, y) OK
#   - v_user_org/v_delegate_org are text → COALESCE(x, '') OK
#
# No previous migration edited. No data deleted. No CASCADE added.
# All prior fixes preserved: v_next_participants uuid[], q.user_id, ON CONFLICT metadata clearing,
#   event types, no meeting/change, GET STACKED DIAGNOSTICS logging, atomicity.
*/
...
BEGIN
  ...
  -- calendar_id is uuid; cast to text before COALESCE to avoid 22P02
