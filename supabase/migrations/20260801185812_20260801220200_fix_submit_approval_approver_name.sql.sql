...
# Resolve approver_name from profiles.full_name for each approver
# Include approver_name, full_name, recipient_greeting in context
# Per-approver idempotency for both minute_approval_requested and minute_resubmitted
*/
  ...
  IF v_is_resubmit THEN
    ...
    LOOP
      IF v_approver_user_id IS DISTINCT FROM v_user_id THEN
        SELECT COALESCE(NULLIF(full_name, ''), 'کاربر') INTO v_approver_name
        FROM public.profiles WHERE user_id = v_approver_user_id LIMIT 1;
