...
begin
  ...
  if v_room.meeting_id is not null then
    ...
    -- Standalone rooms are joinable by any fully-authorized authenticated user
    -- who resolved the room code. The public wrapper still enforces the FULL
    -- app session gate, while lock/ban/waiting-room/capacity checks remain here.
    v_meeting_allowed:=true;
  end if;
  ...
  if v_rbac_role is null and v_room.meeting_id is null then
    -- Match the legacy join_conference_authenticated behavior for a first-time
    -- standalone participant before conference_participants has been created.
  ...
  if v_joined_count+v_reserved_count>=least(v_room.max_participants,20) then
    return jsonb_build_object(
      'ok',false,
      'reason','room_full',
