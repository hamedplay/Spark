/*
# Phase 4 — Fix on_auth_user_created_lifecycle_profile trigger function

## Summary
Replaces the broken `public.on_auth_user_created_lifecycle_profile()` trigger function.
The previous version referenced nonexistent tables `public.challenges` and
`public.audit_events`, which caused every new auth user with a confirmed phone to
raise a check_violation exception and roll back the entire auth user creation.

This migration rewrites the function body to:
- Read challenge metadata from `NEW.raw_app_meta_data` (registration_challenge_id,
  registration_claim_id, registration_identity_hash) for public registration.
- Validate the challenge against `public.public_registration_challenges` with
  `FOR UPDATE` (status=processing, claim match, identity match, not expired,
  created_user_id IS NULL).
- Atomically insert the profile row and consume the challenge in the same
  transaction.
- Use `public.security_audit_events` for audit (never `public.audit_events`).
- Handle admin-created users (registration_flow=admin_created_v1) with full
  profile metadata from raw_user_meta_data.

## Safety
- No prior migration file is modified.
- The existing trigger on auth.users is preserved (only the function body changes).
- No data is deleted, reset, truncated, or modified.
- No edge function is changed.
- No RLS policy is changed.
- GUCs app.account_lifecycle_write and app.profile_completion_write are set
  transaction-local and reset in the EXCEPTION handler.
- No ON CONFLICT DO NOTHING — duplicate profile inserts surface as hard errors
  so the entire auth user creation rolls back.
*/
