/*
# Integrate _sync_minutes_decisions into create_minutes_draft and update_minutes_draft

## Summary

The frontend was calling `_sync_minutes_decisions` directly after create/update,
but `authenticated` lacks EXECUTE on that internal helper — causing a regression
where decisions silently fail to save.

This migration adds an optional `p_decisions jsonb` parameter to both
`create_minutes_draft` and `update_minutes_draft`. When provided (non-null),
the function calls `_sync_minutes_decisions` internally within the same
transaction. If the sync fails, the entire transaction fails — no false success.

The frontend can now stop calling `_sync_minutes_decisions` directly.

## Changes

### create_minutes_draft
  - New signature: (p_payload jsonb, p_decisions jsonb DEFAULT NULL)
  - After inserting the minute + participants + agenda, if p_decisions is not null,
    call `_sync_minutes_decisions(v_minute_id, p_decisions)`.
  - If sync raises an exception, the outer EXCEPTION block catches it and returns
    success=false (transaction rolls back).

### update_minutes_draft
  - New signature: (p_minute_id uuid, p_expected_updated_at timestamptz, p_payload jsonb, p_decisions jsonb DEFAULT NULL)
  - After updating the minute + participants + agenda, if p_decisions is not null,
    call `_sync_minutes_decisions(p_minute_id, p_decisions)`.
  - Same error handling.

## Backward compatibility
  - The new parameter has a DEFAULT of NULL, so existing callers that don't pass
    p_decisions continue to work unchanged.
  - The function is NOT SECURITY DEFINER (same as before), so no grant changes needed.
  - `authenticated` already has EXECUTE on both functions.

## Idempotency
  CREATE OR REPLACE is used. Re-running is safe.
...
-- ═══════════════════════════════════════════════════════════════════════
-- create_minutes_draft — add p_decisions parameter
-- ═══════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.create_minutes_draft(p_payload jsonb, p_decisions jsonb DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SET search_path TO ''
AS $function$
...
-- Sync decisions within the same transaction if provided
IF p_decisions IS NOT NULL THEN
  PERFORM public._sync_minutes_decisions(v_minute_id, p_decisions);
END IF;
...
-- ═══════════════════════════════════════════════════════════════════════
-- update_minutes_draft — add p_decisions parameter
-- ═══════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.update_minutes_draft(p_minute_id uuid, p_expected_updated_at timestamp with time zone, p_payload jsonb, p_decisions jsonb DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SET search_path TO ''
AS $function$
...
-- Sync decisions within the same transaction if provided
IF p_decisions IS NOT NULL THEN
  PERFORM public._sync_minutes_decisions(p_minute_id, p_decisions);
END IF;
...
-- Re-grant EXECUTE to authenticated (signature changed, so grants need refresh)
