/*
# Phase 3C Runtime and Pagination Fixes

## Summary
Fixes three functions from the prior Phase 3C migration:
1. set_user_security_admin - concurrency, validation order, grant consumption order
2. get_security_admin_management_state - pagination structure, search precedence, history limit
3. get_security_audit_page - pagination structure, array trimming, cursor
...
BEGIN
  ...
  -- 5. Global advisory lock (constant, NOT target-specific)
  PERFORM pg_advisory_xact_lock(987654321);

  -- 6. Re-check actor after acquiring lock
  ...
  -- 7. Target lookup with FOR UPDATE
  SELECT
  ...
  -- 8. Target validation (BEFORE grant consumption)

  -- 8a. No-op check: requested value matches current state
  ...
  -- 8c. Revoke operation validation
  ELSE
    -- Last active security admin protection (BEFORE grant consumption)
  ...
  -- 9. Step-up grant validation and consumption (AFTER all target validation)
  ...
  -- 9a. Consume grant with consumed_at IS NULL guard
  ...
  -- 10. Final optimistic version check (AFTER grant consumption)
  ...
  -- 12. History (reason stored only here, not in audit metadata)
  ...
  -- 13. Insert audit event (metadata has no raw reason, only a boolean flag)
...
-- 2. Fixed get_security_admin_management_state
...
BEGIN
  ...
  -- Build user list using CTE with proper pagination
  ...
  SELECT count(*) > v_limit INTO v_has_more
  FROM page_plus_one;
  ...
  -- Role history: limit BEFORE aggregate, then join
  SELECT jsonb_agg(jsonb_build_object(
    'id', h.id,
...
-- 3. Fixed get_security_audit_page
...
BEGIN
  ...
  -- Query with CTE-based keyset pagination
  ...
  ) ORDER BY vp.created_at DESC, vp.id DESC) INTO v_events
  FROM visible_page vp
