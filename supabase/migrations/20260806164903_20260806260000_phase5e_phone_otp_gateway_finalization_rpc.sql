-- Phase 5E-C5B: Atomic Phone OTP Gateway Finalization RPC
-- Creates authorize_phone_otp_gateway_session_v1: validates session, matches challenge,
-- inserts gateway authorization, and consumes challenge atomically.
-- No edge function, no UI, no password RPC changes.

CREATE FUNCTION public.authorize_phone_otp_gateway_session_v1(
  p_session_id uuid,
  p_user_id uuid,
  p_challenge_id uuid,
  p_claim_id uuid,
...
BEGIN
  ...
  IF NOT v_session_found
     ...
     OR (v_session_not_after IS NOT NULL AND v_session_not_after <= v_now)
