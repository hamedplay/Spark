/*
# Fix next_7_days deadline filter: exactly 7 calendar days, today through today+6 inclusive.
# Previously used today+7 as the upper bound, which included 8 days.
# Only the next_7_days branch is changed in both RPCs.
# this_week, aliases, all other filters, and all other logic remain unchanged.
*/
  ...
  WITH base AS (
    ...
    WHERE d.primary_owner_user_id = v_user_id
      ...
      AND (
        ...
        (v_dl = 'next_7_days' AND d.due_date IS NOT NULL AND d.due_date >= v_today::date AND d.due_date <= (v_today::date + 6) AND d.status NOT IN ('completed','stopped')) OR
