/*
# Fix: Allow NULL result_type on minutes_agenda_results

## Problem
The frontend moved result fields (discussion_result, result_type, additional_notes)
from agenda items to decisions. The agenda payload now sends result_type = null.
However, two things rejected NULL:

1. CHECK constraint `minutes_agenda_results_result_type_check` required result_type
   to be one of ('discussion','action','resolution','deferred','no_result') — NULL
   was not allowed because the column was NOT NULL.
2. RPC validation in `create_minutes_draft` and `update_minutes_draft` raised
   `INVALID_RESULT_TYPE` when `v_a_result_type IS NULL`.

## Changes

### 1. Column nullability
- ALTER `minutes_agenda_results.result_type` to be NULLABLE (DROP NOT NULL).

### 2. CHECK constraint
- Drop and recreate `minutes_agenda_results_result_type_check` to allow NULL:
  `result_type IS NULL OR result_type IN ('discussion','action','resolution','deferred','no_result')`
  Non-NULL values are still validated against the same enum.

### 3. RPC: create_minutes_draft
- Replace the validation block that raised INVALID_RESULT_TYPE on NULL with
  one that only rejects non-NULL invalid values.

### 4. RPC: update_minutes_draft
- Same change as create_minutes_draft.

## Security
- No RLS, permission, or SECURITY DEFINER changes.

## Data safety
- No data is deleted or rewritten.
- The constraint is a strict superset: old valid values stay valid, NULL becomes valid.
*/

-- 1. Make result_type nullable
ALTER TABLE public.minutes_agenda_results
  ALTER COLUMN result_type DROP NOT NULL;

-- 2. Replace CHECK constraint to allow NULL
ALTER TABLE public.minutes_agenda_results
  DROP CONSTRAINT IF EXISTS minutes_agenda_results_result_type_check;

ALTER TABLE public.minutes_agenda_results
  ADD CONSTRAINT minutes_agenda_results_result_type_check
  CHECK (result_type IS NULL OR result_type IN ('discussion', 'action', 'resolution', 'deferred', 'no_result'));

-- 3. Recreate create_minutes_draft with relaxed result_type validation
...
BEGIN
  ...
  IF v_agenda_arr IS NOT NULL THEN
    FOR i IN 0..jsonb_array_length(v_agenda_arr) - 1 LOOP
      ...
      IF v_a_result_type IS NOT NULL
      AND v_a_result_type NOT IN ('discussion', 'action', 'resolution', 'deferred', 'no_result') THEN
  ...
  INSERT INTO public.minutes (
    ...
    notes, confidentiality, status, created_by_user_id
  ) VALUES (
    ...
    v_notes, v_confidentiality, 'draft', v_user_id
  )
  ...
  RETURN jsonb_build_object('success', true, 'minute_id', v_minute_id, 'message', 'Draft created successfully');

  EXCEPTION
  WHEN unique_violation THEN
    GET STACKED DIAGNOSTICS v_constraint_name = CONSTRAINT_NAME;
    IF v_constraint_name = 'minutes_meeting_id_key' THEN
      RETURN jsonb_build_object('success', false, 'minute_id', null,
      'error_code', 'MINUTES_ALREADY_EXISTS', 'sqlstate', '23505',
      'message', 'برای این جلسه قبلاً صورتجلسه ثبت شده است');
    ELSIF v_constraint_name = 'minutes_participants_minute_user_unique' THEN
      RETURN jsonb_build_object('success', false, 'minute_id', null,
      'error_code', 'DUPLICATE_INTERNAL_PARTICIPANT', 'sqlstate', '23505',
      'message', 'این شرکت‌کننده قبلاً اضافه شده است');
    ELSIF v_constraint_name = 'minutes_agenda_results_minute_agenda_unique' THEN
      RETURN jsonb_build_object('success', false, 'minute_id', null,
      'error_code', 'DUPLICATE_AGENDA_ITEM', 'sqlstate', '23505',
      'message', 'این دستور جلسه قبلاً اضافه شده است');
    ELSE
      RETURN jsonb_build_object('success', false, 'minute_id', null,
      'error_code', 'INTERNAL_ERROR', 'sqlstate', '23505',
      'message', 'خطای داخلی در ایجاد پیش‌نویس');
    END IF;
  WHEN SQLSTATE 'P0001' THEN
    GET STACKED DIAGNOSTICS v_msg_text = MESSAGE_TEXT;
    RETURN jsonb_build_object('success', false, 'minute_id', null,
    'error_code', v_msg_text, 'sqlstate', 'P0001', 'message', v_msg_text);
  WHEN invalid_text_representation OR numeric_value_out_of_range OR datatype_mismatch THEN
    ...
    RETURN jsonb_build_object('success', false, 'minute_id', null,
    'error_code', 'PAYLOAD_INVALID', 'sqlstate', v_diag_sqlstate,
    'message', 'ساختار اطلاعات ارسالی معتبر نیست');
  WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS v_diag_sqlstate = RETURNED_SQLSTATE;
    RETURN jsonb_build_object('success', false, 'minute_id', null,
    'error_code', 'INTERNAL_ERROR', 'sqlstate', v_diag_sqlstate,
    'message', 'خطای داخلی در ایجاد پیش‌نویس');
END;
$function$;

-- 4. Recreate update_minutes_draft with relaxed result_type validation
CREATE OR REPLACE FUNCTION public.update_minutes_draft(p_minute_id uuid, p_expected_updated_at timestamptz, p_payload jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SET search_path TO ''
AS $function$
