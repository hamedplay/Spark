-- Phase 5E-B3: Atomic Phone OTP Rate Limit RPC
-- Creates the consume_phone_otp_login_rate_limit_v2 function.
-- No new tables, no triggers, no policies, no views, no edge function changes.

CREATE FUNCTION public.consume_phone_otp_login_rate_limit_v2(
  p_purpose text,
  p_phone_hash text,
  p_ip_hash text,
...
BEGIN
  -- Input validation
  IF p_purpose NOT IN ('phone_otp_login_request', 'phone_otp_login_verify') THEN
  ...
  IF p_window_seconds < 30 OR p_window_seconds > 86400 THEN
    RAISE EXCEPTION 'INVALID_RATE_LIMIT_CONFIGURATION' USING ERRCODE = '22023';
  END IF;
