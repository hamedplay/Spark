-- Atomic bulk participant sync for recurring/bulk meeting edits.
-- Additive migration: new RPC only. Reuses existing UNIQUE(meeting_id,user_id) constraint.
...
BEGIN
  ...
  -- Deduplicate and sanitize next participants (excluding organizer per create-flow convention)
  v_next_ids := ARRAY(
    SELECT DISTINCT x FROM unnest(COALESCE(p_participant_user_ids, ARRAY[]::uuid[])) AS t(x)
    WHERE x IS NOT NULL
  );

  -- Validate all next participants once: exist, same org, active, not hidden
  ...
  -- Verify caller is organizer or admin for ALL meetings; lock all rows
  ...
  FOREACH v_meeting_id IN ARRAY p_meeting_ids LOOP
    ...
    IF array_length(v_added_ids, 1) > 0 THEN
      ...
      SELECT v_meeting_id, u.id, 'pending'
