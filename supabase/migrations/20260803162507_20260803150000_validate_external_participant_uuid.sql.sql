/*
# Defensively validate external participant UUIDs in minutes draft RPCs
#
# 1. create_minutes_draft(jsonb, jsonb)
#    - Before casting external_participants[].id to uuid, validate it is a valid UUID.
#    - Invalid values return error_code 'INVALID_EXTERNAL_PARTICIPANT_ID' instead of raw PostgreSQL errors.
#    - Raw PostgreSQL message, input value, and constraint name are NOT returned to the caller.
#
# 2. update_minutes_draft(uuid, timestamptz, jsonb, jsonb, uuid[], uuid[])
#    - Same validation for external_participants[].id.
#    - Same validation for p_deleted_external_participant_ids array elements.
#    - Backward-compat 4-param wrapper is preserved (calls 5-param with empty arrays).
#
# 3. _sync_minutes_decisions(uuid, jsonb, uuid[])
#    - Before casting external_responsible_participant_id to uuid, validate it.
#    - Invalid values return 'INVALID_EXTERNAL_PARTICIPANT_ID'.
#    - Existing scope/ownership checks (belongs to same minute) are preserved.
#
# No existing function signatures changed.
# SECURITY DEFINER, search_path = '', and GRANT/REVOKE preserved on all functions.
# No previous migration edited. No data deleted, repaired, reset, or truncated.
# No existing IDs changed.
*/

-- ════════════════════════════════════════════════════════════════════════════
-- Helper: validate UUID string without throwing
-- ════════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public._is_valid_uuid(v text)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
SECURITY DEFINER
...
REVOKE EXECUTE ON FUNCTION public._is_valid_uuid(text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public._is_valid_uuid(text) FROM anon;
REVOKE EXECUTE ON FUNCTION public._is_valid_uuid(text) FROM authenticated;

-- ════════════════════════════════════════════════════════════════════════════
-- 1. Redefine _sync_minutes_decisions with UUID validation
-- ════════════════════════════════════════════════════════════════════════════
  ...
  IF jsonb_array_length(v_arr) > 0 THEN
    FOR i IN 0..jsonb_array_length(v_arr) - 1 LOOP
      ...
      ELSIF v_responsible_party_type = 'external' THEN
        ...
        END IF;
        -- Validate external_responsible_participant_id BEFORE cast
        v_ext_part_id_text := NULLIF(v_dec->>'external_responsible_participant_id', '');
        IF v_ext_part_id_text IS NOT NULL AND NOT public._is_valid_uuid(v_ext_part_id_text) THEN
          RAISE EXCEPTION 'INVALID_EXTERNAL_PARTICIPANT_ID' USING ERRCODE = 'P0001';
        END IF;
        v_ext_participant_id := v_ext_part_id_text::uuid;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 2. Redefine create_minutes_draft with external participant UUID validation
-- ════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  FOR i IN 0..jsonb_array_length(v_external_parts) - 1 LOOP
    ...
    END IF;
    -- Validate id is a valid UUID BEFORE casting
    v_ep_id_text := NULLIF(v_part->>'id', '');
    IF v_ep_id_text IS NOT NULL AND NOT public._is_valid_uuid(v_ep_id_text) THEN
      RAISE EXCEPTION 'INVALID_EXTERNAL_PARTICIPANT_ID' USING ERRCODE = 'P0001';
    END IF;
  END LOOP;
  END IF;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 3. Redefine update_minutes_draft (5-param) with external participant UUID validation
-- ════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════
-- 4. Preserve backward-compat 4-param wrapper
-- ════════════════════════════════════════════════════════════════════════════
