-- Phase 5E-C1: Atomic Phone OTP Challenge Creation RPC
-- Creates the create_phone_otp_login_challenge_v2 function.
-- No new tables, no triggers, no policies, no views, no edge function changes.

CREATE FUNCTION public.create_phone_otp_login_challenge_v2(
  p_challenge_id uuid,
  p_user_id uuid,
  p_phone_hash text,
  p_otp_hash text,
...
BEGIN
  ...
  IF v_last_active IS NOT NULL AND v_last_active.resend_available_at > v_now THEN
