/*
# Add decision_assigned producer to _sync_minutes_decisions

When a decision is created with a new owner or owner changes, emit decision_assigned event.
Only the new owner gets notified (not the old owner).
Actor exclusion: if actor is the owner, no notification.
*/
...
BEGIN
  ...
  SELECT status, created_by_user_id, title, revision_number
  ...
  FOR i IN 0..jsonb_array_length(v_arr) - 1 LOOP
    ...
    -- Only notify if: owner exists, owner is not the actor, and owner changed
    IF v_owner IS NOT NULL AND v_owner IS DISTINCT FROM v_user_id THEN
      IF v_existing_owner IS NULL OR v_existing_owner IS DISTINCT FROM v_owner THEN
        v_idempotency := 'decision:' || COALESCE(v_dec_id::text, '') || ':decision_assigned:' || v_owner::text || ':' || COALESCE(v_revision::text, '0');
        v_context := jsonb_build_object(
          ...
          'decision_link', '#minutes-my-decisions?decision=' || COALESCE(v_dec_id::text, ''),
          'actor_name', '',
