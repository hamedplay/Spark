-- Fix unnest syntax in sync_meeting_participants_v2 (array dedup and diff queries).
-- Additive: recreates the function with corrected SQL. Does not touch other objects.
...
BEGIN
  ...
  -- Caller must be organizer or admin; lock row to prevent concurrent races
  ...
  -- Validate all next participants: must exist, same org, active, not hidden
...
-- Ensure grants remain correct after recreate
