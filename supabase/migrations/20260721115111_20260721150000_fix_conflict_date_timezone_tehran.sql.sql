-- Fix: conflict date comparison must use Asia/Tehran timezone day, not UTC date component.
-- The UI (parseRequestDateToDateStr, MeetingDetailModal) displays meeting day in Iran timezone (UTC+3:30).
-- The previous accept_meeting_invitation_v2 used left(request_date, 10) which extracts the UTC calendar day,
-- causing missed conflicts for meetings on the same Iran day but different UTC days (e.g. 20:30:00Z vs 07:00:00Z).
-- This additive migration replaces the function with a timezone-aware version.
...
GRANT EXECUTE ON FUNCTION public.accept_meeting_invitation_v2(uuid, boolean) TO service_role;