/*
# Update claim_notification_outbox_rows to also reclaim partial rows for SMS retry

Must DROP first because return type changed (added notification_status, sms_status, sms_sent_at columns).
*/
...
BEGIN
  ...
  FROM (
    ...
    WHERE (
      ...
      OR
      (status = 'partial' AND next_attempt_at <= now())
    )
