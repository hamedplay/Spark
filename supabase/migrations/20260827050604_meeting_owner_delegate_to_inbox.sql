...
begin
  ...
  if found and v_existing.status in ('pending', 'accepted', 'delegated') then
    return jsonb_build_object('success', false, 'error_code', 'DELEGATE_ALREADY_INVITED');
  end if;

  if (
    p_delegate_user_id = any(coalesce(v_meeting.notify_users, '{}'::uuid[]))
    and not found
  ) then
    return jsonb_build_object('success', false, 'error_code', 'DELEGATE_ALREADY_INVITED');
  end if;
...
create or replace function public.assign_meeting_owner_delegate(
  p_meeting_id uuid,
  p_delegate_user_id uuid
)
returns jsonb
...
set search_path to ''
as $$
  select private.assign_meeting_owner_delegate($1::uuid, $2::uuid)
$$;

revoke all on function private.assign_meeting_owner_delegate(uuid, uuid) from public;
revoke all on function public.assign_meeting_owner_delegate(uuid, uuid) from public, anon;
grant execute on function public.assign_meeting_owner_delegate(uuid, uuid) to authenticated;
