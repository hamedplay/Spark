/*
# Update manage_minutes_decision: reminder creation on followup, cancel on completion/stop

1. Changes to existing function
- Add p_remind_at parameter (timestamptz, default null)
- On followup with p_remind_at: cancel previous pending reminder for same decision+recipient,
  then create new pending reminder with source_update_id
- On completion: cancel all pending reminders for this decision
- On status_change to 'stopped': cancel all pending reminders for this decision

2. Security
- No changes to existing security model
- p_remind_at validated: must be in the future if provided
- recipient defaults to primary_owner_user_id

3. Notes
- DROP and recreate function (same parameters + new p_remind_at)
- Reminder creation is inside the same transaction as the followup update
- Previous pending reminders are cancelled (not deleted) with cancelled_at=now()
*/

-- First, add the new parameter to the function signature.
-- We need to recreate the function with the additional parameter.
...
BEGIN
  ...
  -- ── Reminder management ────────────────────────────────────────────────
  -- On followup with p_remind_at: cancel previous pending reminder, create new
  ...
  -- On status_change to 'stopped': cancel all pending reminders
