/*
# Fix array type mismatch in atomic meeting RPCs

## Problem
`notify_users` and `participant_user_ids` are uuid[] columns, but the
ARRAY(SELECT jsonb_array_elements_text(...)) subquery returns text[].
COALESCE cannot mix text[] and uuid[] ("COALESCE could not convert type
uuid[] to text[]").

## Fix
Cast the text arrays to uuid[] explicitly via ::uuid[] before COALESCE,
so the fallback empty array and the subquery share the same type.
...
BEGIN
  ...
  UPDATE public.meetings SET
    ...
    notify_users = COALESCE(ARRAY(SELECT jsonb_array_elements_text(p_payload->'notify_users'))::uuid[], ARRAY[]::uuid[]),
    participant_user_ids = COALESCE(ARRAY(SELECT jsonb_array_elements_text(p_payload->'participant_user_ids'))::uuid[], ARRAY[]::uuid[]),
