/*
# Minutes Phase 3 — corrective: fix minute_published audit never logged

Bug: in _minutes_audit_trigger_fn, the ELSIF chain checks chair_confirmed_at
BEFORE status='published'. When chair publishes, both chair_confirmed_at and
status+published_at change in the same UPDATE, so chair_confirmed wins and
minute_published is never logged.

Fix: check minute_published (status='published' + published_at changed)
BEFORE chair_confirmed. This ensures the most significant terminal action
is recorded. chair_confirmed still fires when only chair_confirmed_at
changes without a publish (defensive; not currently a separate step but
