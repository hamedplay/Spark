CREATE OR REPLACE FUNCTION private.sync_meeting_participants_bulk_v2(
  p_meeting_ids uuid[],
...
BEGIN
  ...
  -- The organizer owns the meeting and must never be synchronized as a participant.
  -- Exclude every organizer represented in this bulk edit before calculating diffs.
  v_next_ids := ARRAY(
    SELECT DISTINCT x
    FROM unnest(COALESCE(p_participant_user_ids, ARRAY[]::uuid[])) AS t(x)
    WHERE x IS NOT NULL
      AND NOT EXISTS (
        SELECT 1
        FROM public.meetings AS owner_meeting
        WHERE owner_meeting.id = ANY(p_meeting_ids)
          AND owner_meeting.user_id = x
      )
  );
  ...
  FOREACH v_meeting_id IN ARRAY p_meeting_ids LOOP
    ...
    v_previous_ids := ARRAY(
      SELECT DISTINCT x
      FROM unnest(v_previous_ids) AS t(x)
    ...
    SET participant_user_ids = v_next_ids
    WHERE m.id = v_meeting_id;
...
-- Repair existing rows produced by the same invariant violation.
UPDATE public.meetings AS m
SET participant_user_ids = ARRAY(
  SELECT participant_id
  FROM unnest(COALESCE(m.participant_user_ids, ARRAY[]::uuid[])) AS p(participant_id)
  WHERE participant_id IS NOT NULL
    AND participant_id <> m.user_id
)
WHERE m.user_id = ANY(COALESCE(m.participant_user_ids, ARRAY[]::uuid[]));

DELETE FROM public.meeting_inbox AS mi
USING public.meetings AS m
WHERE m.id = mi.meeting_id
  AND mi.user_id = m.user_id
  AND mi.delegated_by_user_id IS NULL;

-- Public wrapper invokes this private implementation as the authenticated caller.
GRANT EXECUTE ON FUNCTION private.decline_meeting_invitation_v2(uuid) TO authenticated;

-- Organizer receives a change notice, never a participant invitation, when participants are added.
...
VALUES (
  ...
  'change',
  'organizer',
  'تغییر شرکت‌کنندگان جلسه',
  'همکار گرامی {{full_name}}، شرکت‌کننده {{participant_name}} به جلسه «{{meeting_subject}}» در تاریخ {{meeting_date}} ساعت {{meeting_time}} اضافه شد.',
  ARRAY['full_name','participant_name','meeting_subject','meeting_date','meeting_time']::text[],
  true,
  now()
)
ON CONFLICT (category, event_type, audience) DO UPDATE
