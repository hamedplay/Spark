...
BEGIN
  ...
  BEGIN
    ...
    VALUES (p_webhook_id, 'processing', now() + interval '30 seconds', 1)
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Table might not exist — fail-closed
    RAISE;
  END;
...
-- ── Atomic consume RPC ──────────────────────────────────────────────────
-- Atomically checks both phone and IP limits, inserts if allowed.
...
BEGIN
  -- Atomic counts within the window
  ...
  -- Insert the rate-limit entry atomically
