...
describe('Phase 5B-3 — Edge Gate Coverage', () => {
  ...
  describe('check-phone-password-reset-runtime', () => {
    ...
    it('checks is_admin and canonical account_status via service role', () => {
      assert.ok(checkResetSrc.includes('is_admin'), 'must check is_admin');
      assert.ok(checkResetSrc.includes('account_status'), 'must check account_status');
      assert.ok(!checkResetSrc.includes('profile.is_active'), 'must not authorize with legacy is_active');
    });
