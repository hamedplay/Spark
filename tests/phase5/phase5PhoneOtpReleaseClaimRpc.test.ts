...
describe('Phase 5E-C4B — Release Phone OTP Claim RPC', () => {
  it('migration file exists on disk', () => {
    assert.ok(releaseMigration, 'phase5e_phone_otp_release_claim_rpc migration must exist');
  });

  it('has exact function signature and return type', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
    assert.ok(/CREATE\s+FUNCTION\s+public\.release_phone_otp_login_challenge_v2\s*\(/i.test(sql),
  ...
  it('invalid input raises 22023 with INVALID_RELEASE_CONFIGURATION', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
    assert.ok(/RAISE\s+EXCEPTION\s+'INVALID_RELEASE_CONFIGURATION'/i.test(sql),
      'must raise INVALID_RELEASE_CONFIGURATION');
  ...
  it('pending/sent/no-claim is idempotent success', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
    assert.ok(/v_status\s*=\s*'pending'\s+AND\s+v_delivery_status\s*=\s*'sent'\s+AND\s+v_existing_claim_id\s+IS\s+NULL\s+AND\s+v_claim_expires_at\s+IS\s+NULL\s+AND\s+v_consumed_at\s+IS\s+NULL/i.test(sql),
      'must check pending/sent/no-claim/no-consumed for idempotency');
    assert.ok(/'released',\s*true/i.test(sql), 'must return released true');
  ...
  it('consumed state returns ALREADY_CONSUMED', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
  ...
  it('terminal states return INVALID_CHALLENGE_STATE', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
    assert.ok(/v_status\s+IN\s*\(\s*'superseded'\s*,\s*'expired'\s*,\s*'locked'\s*,\s*'delivery_failed'\s*\)/i.test(sql),
      'must check superseded, expired, locked, delivery_failed');
    assert.ok(/'error_code',\s*'INVALID_CHALLENGE_STATE'/i.test(sql),
      'must return INVALID_CHALLENGE_STATE');
  });

  it('only processing/sent can be released', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
  ...
  it('UPDATE has all preconditions in WHERE', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
    assert.ok(/WHERE\s+id\s*=\s*p_challenge_id\s+AND\s+status\s*=\s*'processing'\s+AND\s+delivery_status\s*=\s*'sent'\s+AND\s+claim_id\s*=\s*p_claim_id\s+AND\s+claim_expires_at\s+IS\s+NOT\s+NULL\s+AND\s+consumed_at\s+IS\s+NULL/i.test(sql),
      'UPDATE WHERE must check id, status=processing, delivery_status=sent, claim_id, claim_expires_at IS NOT NULL, consumed_at IS NULL');
  });

  it('transition only changes status, claim fields, and updated_at', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
    assert.ok(/SET\s+status\s*=\s*'pending'\s*,\s*claim_id\s*=\s*null\s*,\s*claim_expires_at\s*=\s*null\s*,\s*updated_at\s*=\s*v_now/i.test(sql),
      'must set status=pending, claim_id=null, claim_expires_at=null, updated_at=v_now');
  });

  it('does not modify attempt_count in UPDATE', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
  ...
  it('does not modify delivery_status in UPDATE', () => {
    assert.ok(releaseMigration);
    const sql = readFileSync(join(migrationsDir, releaseMigration!), 'utf8');
    const updateSections = sql.match(/SET\b[^;]*?WHERE/gis) || [];
    for (const section of updateSections) {
  ...
  it('no formal or comment-only tests exist in this file', () => {
    const testFile = readFileSync(join(root, 'tests', 'phase5', 'phase5PhoneOtpReleaseClaimRpc.test.ts'), 'utf8');
