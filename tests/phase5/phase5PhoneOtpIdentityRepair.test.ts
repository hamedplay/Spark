...
describe('Phase 5E-D6 Fix 4 — Strict Pre-Write Fail-Closed + Safe Repair Responses', () => {
  ...
  it('security-admin gate uses canonical account status', () => {
    assert.ok(/requireFullAuthAccess/.test(funcSrc), 'must require full auth access');
    assert.ok(/is_security_admin/.test(funcSrc), 'must check is_security_admin');
    assert.ok(/account_status/.test(funcSrc), 'must check account_status');
    assert.ok(!/profile\?\.is_active/.test(funcSrc), 'must not use legacy profile.is_active for authorization');
  });
