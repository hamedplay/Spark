...
test('normalizes raw Supabase snake_case TOTP factors without admitting other factor types', async () => {
  const { normalizeSupabaseTotpFactors } = await import(
    '../../src/features/auth/services/totpFactors'
  );
  const factors = normalizeSupabaseTotpFactors([
    {
      id: 'factor-1', friendly_name: 'totp-test', factor_type: 'totp',
      status: 'verified', created_at: '2026-08-12T10:00:00Z', updated_at: '2026-08-12T10:00:00Z',
    },
    {
      id: 'factor-2', factor_type: 'totp', status: 'unverified',
      created_at: '2026-08-12T10:01:00Z', updated_at: '2026-08-12T10:01:00Z',
    },
    {
      id: 'factor-3', factor_type: 'webauthn', status: 'verified',
      created_at: '2026-08-12T10:02:00Z', updated_at: '2026-08-12T10:02:00Z',
    },
    {
      id: 'factor-4', factor_type: 'totp', status: 'verified',
      created_at: '2026-08-12T10:03:00Z', updated_at: '2026-08-12T10:03:00Z',
    },
  ]);

  assert.deepEqual(factors[0], {
    id: 'factor-1', friendlyName: 'totp-test', factorType: 'totp',
    status: 'verified', createdAt: '2026-08-12T10:00:00Z',
  });
  assert.equal(factors.length, 3, 'WebAuthn must not be included as TOTP');
  assert.equal(factors.filter((factor) => factor.status === 'verified').length, 2,
    'the challenge can select only the two verified TOTP factors');
  assert.ok(factors.every((factor) => factor.factorType === 'totp'));
});
